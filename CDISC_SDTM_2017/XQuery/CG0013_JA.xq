...
(: TODO: get the list of variables using the CDISC-Library API RESTful web services.
Complication: authentication :)
(: SDTM version 1.4 :)
...
(: Trial Summary (TS) :)
(: 2017-06-11: added TSVAL1 to TSVAL9 for backward SDTM-XPT compatibility  :)
let $trialsummaryvars := ('STUDYID','DOMAIN','TSSEQ','TSGRPID','TSPARMCD','TSPARM','TSVAL','TSVAL1','TSVAL2','TSVAL3','TSVAL4','TSVAL5','TSVAL6','TSVAL7','TSVAL8','TSVAL9','TSVALNF','TSVALCD','TSVCDREF','TSVCDVER')
...
(: let $base := 'file:///c:/download/Stefan_Dyla_Conformance_Rules_implementation/' :)
...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    let $domaincode := (
        if(starts-with($dataset/@Name,'SUPP')) then 'SUPPQUAL'
        else if($dataset/@Domain) then $dataset/@Domain
        else $dataset/@Name
    )
    ...
    let $findingsaboutvars := local:replacedash($findingsaboutvars,$domaincode)
    ...
    let $standardvars := (
        ...
        else if($domaincode = 'SUPPQUAL') then $suppqualvars
        ...
        (:2017-06-16: addition for FA datasets  :)
        else if(starts-with($domaincode,'FA') and upper-case($domainclass) = 'FINDINGS') then
            functx:value-union(functx:value-union($identifiers,$findingsaboutvars),$timingvars)   
    ...
    for $var in $vararray
        ...
        return <error rule="CG0013" dataset="{data($datasetname)}" variable="{data($var)}" rulelastupdate="2017-06-16">Variable {data($var)} is not in the SDTM model (v.1.4) for domain {data($domaincode)} in class {data($domainclass)} </error>
