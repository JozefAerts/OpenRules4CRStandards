...
(: A web secvice is used to retrieve the required variables for each domain/dataset :)
xquery version "3.0";
...
(: temporary: TODO the webservice must be changed :)
let $webservicebase := 'http://www.xml4pharmaserver.com:8080/CDISCCTService/rest/VariablesForDomain/SDTM/'
...
for $itemgroup in $datasets
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        for $varoid in $requiredoids
            ...
            return <error rule="CG0014" dataset="{$dsname}" variable="{data($varname)}" rulelastupdate="2017-02-04" recordnumber="{$recnum}">No data found for required variable {data($varname)} in record number {data($recnum)} in dataset {data($datasetname)}</error> 
