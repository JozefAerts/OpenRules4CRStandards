...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Domain and not(starts-with(@Name,'SUPP'))]
    ...
    where $count > 1 and ($numchars < 3 or $numchars > 4)
        return <error rule="CG0017" dataset="{data($name)}" rulelastupdate="2017-02-04">Split dataset {data($name)} has a dataset/file name '{data($filename)}' with less than 3 or more than 4 characters</error>			
