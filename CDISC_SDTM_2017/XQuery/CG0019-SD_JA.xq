...
let $datasetname := 'LB' :)
(: iterate over the given dataset in the define.xml :)
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Domain=$datasetname or starts-with(@Name,$datasetname)]
    ...
    (: 2019-08-07: worst case scenario there are no keys defined :)
    return
    if (count($keyoids) = 0) then
        <error rule="CG0019" dataset="{data($name)}" rulelastupdate="2019-08-07">No keys have been defined for dataset {data($name)} in the define.xml!</error>
    else 
    ...
    for $record in $datasetdoc//odm:ItemGroupData 
        ...
        for $record2 in $datasetdoc//odm:ItemGroupData[@data:ItemGroupDataSeq > $recordnum]
            ...
            return <error rule="CG0019" dataset="{data($name)}" rulelastupdate="2019-08-07">Records {data($recordnum)} and {data($recordnum2)} have the same values for the key variables as defined in the define.xml</error>			
