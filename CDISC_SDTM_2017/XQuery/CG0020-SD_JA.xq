...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Domain=$datasetname or starts-with(@Name,$datasetname)]
    ...
    for $record in $datasetdoc[count($codedoids)>0]//odm:ItemGroupData
        ...
        for $codeditem in $record/odm:ItemData[functx:is-value-in-sequence(@ItemOID,$codedoids)]
            ...
            return <error rule="CG0020" dataset="{data($name)}" variable="{data($varname)}" recordnumber="{data($recnum)}" rulelastupdate="2017-03-20">Value for variable {data($varname)}: value {data($value)} not found in associated codelist {data($codelistoid)} and is not 'MULTIPLE'</error>						
