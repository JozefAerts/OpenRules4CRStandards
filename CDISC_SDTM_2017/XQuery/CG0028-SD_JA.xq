...
(: Rule CG0028-SD The value of Sequence Number (--SEQ) variable must be unique for each record within a domain and within each Unique Subject Identifier (USUBJID) or Pool Identifier (POOLID) variables value when they are present in the domain :)
(: TODO: must be done OVER (spanning) datasets within the same domain :)
xquery version "3.0";
...
(: The following statement also allows to iterate over all datasets in a single class (def:Class)  :)
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname or @def:Class=$datasetname]
...
let $datasets := concat($base,$dsname) (: result is a set of datasets :)
   ...
   let $orderedrecords := (
    ...
    for $group in $orderedrecords
        ...
        where count($group/odm:ItemGroupData) > 1
            return <error rule="CG0028" dataset="{data($itemgroupname)}" variable="{data($seqname)}" rulelastupdate="2017-02-04" recordnumber="{data($recnum)}">The record with USUBJID = {data($usubjid)} and {data($seqname)} is not unique in the dataset {data($itemgroupname)}. The following records have the same combination of USUBJID and {data($seqname)}: records number {data($recnums)}</error>
