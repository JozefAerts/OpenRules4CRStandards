...
(: 2019-08-11: create an interim structure containing SV USUBJID-VISITNUM pairs.
 This avoids that the SV dataset needs to be opened over and over again. 
 It reduces the query time with a factor of about 4 :)
let $visitnumpairs := (
...
for $itemgroup in $definedoc//odm:ItemGroupDef[not(@Name='SV') and @Name=$datasetname]
    (: get the location :)
	...
	This new algorithm further the reduces the query time further by a factor 4.5. 
	Gain again the old algoirithm: about a factor 20 :)
    let $orderedrecords := (
    ...
    for $group in $orderedrecords
        ...
        for $record in $group/odm:ItemGroupData 
            ...
            return <error rule="CG0034" rulelastupdate="2019-08-11" dataset="{data($datasetname)}" variable="VISITNUM" recordnumber="{data($recnum)}">Combination of USUBJID='{data($usubjid)}' and VISITNUM='{data($visitnum)} in dataset {data($datasetname)} is not found in the SV domain</error>					
