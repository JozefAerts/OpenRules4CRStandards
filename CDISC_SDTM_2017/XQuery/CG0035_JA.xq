...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    (: each group has the same value for VISITNUM and VISIT  :)
        ...
        for $index2 in (($index1+1) to fn:count($orderedrecords))
            ...
            return <error rule="CG0035" rulelastupdate="2019-08-07" dataset="{data($name)}" variable="VISITNUM" recordnumber="{data($recnum2)}">Combination of VISIT='{data($visit2)}' and VISITNUM={data($visitnum2)} in dataset {data($name)} is inconsistent: 
