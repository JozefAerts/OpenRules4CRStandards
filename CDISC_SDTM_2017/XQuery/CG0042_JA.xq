...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Domain='AE' or starts-with(@Name,'AE')]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData
        ...
        return <error rule="CG0041" dataset="{data($name)}" variable="AESER" rulelastupdate="2017-02-05" recordnumber="{data($recnum)}">No qualifiers AESCAN, AESCONG, AESDISAB, AESDTH, AESHOSP, AESLIFE or AESMIE set to 'Y', when AE is Serious in dataset {data($datasetname)}</error>				
