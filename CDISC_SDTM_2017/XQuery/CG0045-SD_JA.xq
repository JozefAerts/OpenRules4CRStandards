...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname]
    ...
    let $recnum := $record/@data:ItemGroupDataSeq
        ...
        return <error rule="CG0045" rulelastupdate="2019-08-11" dataset="{data($name)}" variable="{data($enrtptname)}" recordnumber="{data($recnum)}">Missing value for {data($enrtptname)}, when {data($entptname)} is populated (not null), value = {data($entptvalue)}</error>				
