...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    let $recnum := $record/@data:ItemGroupDataSeq
        ...
        return <error rule="CG0045" rulelastupdate="2017-03-05" dataset="{data($name)}" variable="{data($enrtptname)}" recordnumber="{data($recnum)}">Missing value for {data($enrtptname)}, when {data($entptname)} is populated (not null), value = {data($entptvalue)}</error>				
