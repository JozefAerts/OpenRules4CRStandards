...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    let $recnum := $record/@data:ItemGroupDataSeq
        ...
        return <error rule="CG0046" dataset="{data($name)}" rulelastupdate="2019-08-11" variable="{data($entptname)}" recordnumber="{data($recnum)}">Missing value for {data($entptname)}, when {data($enrtptname)} is populated (not null), value = {data($enrtptvalue)}</error>	
