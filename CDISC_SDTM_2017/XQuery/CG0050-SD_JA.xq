...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname]
    ...
    let $recnum := $record/@data:ItemGroupDataSeq
        ...
        return <error rule="CG0050" dataset="{data($name)}" rulelastupdate="2019-08-11" recordnumber="{data($recnum)}">Missing value for {data($decodname)}, when {data($ptcdname)} is populated (not null), value = {data($ptcdvalue)} in dataset {data($name)}</error>				
