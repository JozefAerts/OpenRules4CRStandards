...
(: iterate over all the EVENTS datasets :)
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[@def:Class='EVENTS']
    ...
    let $recnum := $record/@data:ItemGroupDataSeq
        ...
        return <error rule="CG0050" dataset="{data($name)}" rulelastupdate="2017-03-05" recordnumber="{data($recnum)}">Missing value for {data($decodname)}, when {data($ptcdname)} is populated (not null), value = {data($ptcdvalue)} in dataset {data($name)}</error>				
