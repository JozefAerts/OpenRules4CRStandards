...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    return <error rule="CG0057" variable="{data($enrtptname)}" dataset="{$name}" rulelastupdate="2017-02-05">{data($enrtptname)} is not present although {data($entptname)} is present</error>	
