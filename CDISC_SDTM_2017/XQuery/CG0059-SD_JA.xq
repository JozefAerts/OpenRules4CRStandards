...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    return <error rule="CG0059" variable="{data($strtptname)}" dataset="{$name}" rulelastupdate="2019-08-11" recordnumber="{data($recnum)}">No value for {data($strtptname)} was found although there is a value for {data($sttptname)}={data($sttptvalue)}</error>			
