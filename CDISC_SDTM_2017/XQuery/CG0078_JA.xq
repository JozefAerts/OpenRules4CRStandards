...
for $record in $mhdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$mhendtcoid and @Value != '']]
    ...
    return <error rule="CG0078" variable="MHENDTC" dataset="{data($name)}" rulelastupdate="2017-02-09" recordnumber="{data($recnum)}">MHENDTC={data($mhendtc)} is after or on RFSTDTC={data($rfstdtc)}</error>
