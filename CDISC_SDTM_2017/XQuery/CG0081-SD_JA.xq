...
(: Applies to: EVENTS,INTERVENTIONS, NOT(DS, DV, EX) :)
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS'][not(@Name='DS') and not(@Name='DV') and not(@Name='EX')][@Name=$datasetname]
    ...
    return <error rule="CG0081" variable="{data($statname)}" dataset="{$name}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-11">Invalid value for {data($statname)}='{data($stat)}' for {data($prespname)}='Y' and {data($occurname)}=null. Value for {data($statname)} expected is 'NOT DONE'</error>				
