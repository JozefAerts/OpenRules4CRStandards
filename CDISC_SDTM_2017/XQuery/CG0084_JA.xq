...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[starts-with(@Name,'LB') or starts-with(@Name,'AE')]
    ...
    return 
    <error rule="CG0084" rulelastupdate="2017-02-10" dataset="{data($name)}" variable="{data($toxname)}">{data($toxname)} is present, although {data($toxgrname)} is absent in dataset {data($name)}</error>	
