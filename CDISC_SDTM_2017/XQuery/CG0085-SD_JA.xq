...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS' and not(@Name='EX') and not(@Name='DS') and not(@Name='DV')][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$prespoid]]
        ...
        where not($presp='Y')
        return
        <error rule="CG0085" rulelastupdate="2019-08-11" dataset="{data($name)}" variable="{data($prespname)}" recordnumber="{data($recnum)}">{data($prespname)} is only allowed to be 'Y' or null, {data($prespname)}='{data($presp)}' was found</error>			
