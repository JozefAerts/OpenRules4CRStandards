...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')][@Name=$datasetname]
    ...
    for $record in $datasetdoc[$statoid and $occuroid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$prespoid and not(@Value='Y')] and odm:ItemData[@ItemOID=$statoid]/@Value='NOT DONE']
        ...
        return
            <error rule="CG0087" rulelastupdate="2017-02-10" dataset="{data($name)}" variable="{data($occurname)}" recordnumber="{data($recnum)}">{data($occurname)}='{data($occur)}' for record with {data($prespname)}='{data($presp)}' and {data($statname)}='NOT DONE'. {data($occurname)} should be null</error>			
