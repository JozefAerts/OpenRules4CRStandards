...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$occuroid and @Value!='']]
        ...
        where not($presp='Y')
        return
            <error rule="CG0089" rulelastupdate="2017-02-10" dataset="{data($name)}" variable="{data($prespname)}" recordnumber="{data($recnum)}">{data($prespname)}='{data($presp)}' found for {data($occurname)}='{data($occur)}'. Value expected for {data($prespname)}='Y'</error>			
