...
(: Rule CG0091 - When --TPTNUM present in dataset then --TPT present in dataset :)
xquery version "3.0";
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    return
        <error rule="CG0091" rulelastupdate="2017-02-10" dataset="{data($name)}" variable="{data($tptname)}" >Variable {data($tptnumname)} is present in dataset {data($name)} but {data($tptname)} is not present in the dataset</error>			
