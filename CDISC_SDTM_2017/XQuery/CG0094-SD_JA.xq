...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$reasndoid]]
        ...
        return <error rule="CG0094" dataset="{data($name)}" variable="{data($statname)}" rulelastupdate="2019-08-11" recordnumber="{$recnum}">Missing or invalid value for {data($statname)} when {data($reasndname)} is provided - {data($reasndname)}={data($reasndvalue)} - {data($statname)}={data($statvalue)} in dataset {data($datasetname)}</error>				
