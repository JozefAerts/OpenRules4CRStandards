...
Applicable to all datasets except for EX, AE, DS, DV, IE :)
...
(: Iterate over all datasets except for EX, AE, DS, DV and IE - 
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[not(@Name='EX') and not(starts-with(@Name,'AE')) and not(@Domain='AE') and not(@Name='DS') and not(@Name='DV') and not(@Name='IE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$reasndoid]]
        ...
        return <error rule="CG0094" dataset="{data($name)}" variable="{data($statname)}" rulelastupdate="2017-02-10" recordnumber="{$recnum}">Missing or invalid value for {data($statname)} when {data($reasndname)} is provided - {data($reasndname)}={data($reasndvalue)} - {data($statname)}={data($statvalue)} in dataset {data($datasetname)}</error>				
