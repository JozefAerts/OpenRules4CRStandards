...
(: Rule CG0095 - When --LOC not present in dataset then --LAT not present in dataset
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Domain='MI' or starts-with(@Name,'MI') 
    ...
    return 
        <error rule="CG0095" rulelastupdate="2017-02-10" dataset="{data($name)}" variable="{data($latname)}">Variable {data($locname)} is absent in dataset {data($name)} but {data($latname)} is present in the dataset</error>
