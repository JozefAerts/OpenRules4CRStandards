...
(: When ECOCCUR != 'N' and ECDOSTXT is null then ECDOSE > 0 :)
xquery version "3.0";
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Domain='EC' or starts-with(@Name,'EC')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$ecoccuroid and @Value!='N'] and not(odm:ItemData[@ItemOID=$ecdostxtoid])]
        ...
        return 
            <error rule="CG0100" dataset="{data($name)}" variable="ECDOSE" rulelastupdate="2017-02-10" recordnumber="{$recnum}">ECDOSE is not a number or is a non-positive number where ECOCCUR != 'N' and ECDOSTXT is null. The value found for ECDOSE is '{data($dose)}'</error>			
