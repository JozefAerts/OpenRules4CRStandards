...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[not(odm:ItemData[@ItemOID=$trtvoid])]
        ...
        where $vamt and $vamt!=''
        return
        <error rule="CG0106" dataset="{data($name)}" variable="{data($vamtname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-11">{data($name)}={data($vamt)} but was expected to be null as {data($trtvname)} is null</error>
