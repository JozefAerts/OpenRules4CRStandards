...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Domain='EX' or starts-with(@Name,'EX')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[not(odm:ItemData[@ItemOID=$extrtvoid])]
        ...
        return
        <error rule="CG0108" dataset="{data($name)}" variable="EXVAMTU" recordnumber="{data($recnum)}" rulelastupdate="2017-02-11">EXVAMTU={data($exvamtu)} but was expected to be null as EXTRTV is null</error>			
