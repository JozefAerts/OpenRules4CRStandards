...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in doc($datasetlocation)[$doseoid and $dostxtoid]//odm:ItemGroupData
        ...
        return <error rule="CG0110" dataset="{data($name)}" variable="{data($dosename)}" rulelastupdate="2019-08-11" recordnumber="{data($recnum)}">{data($dosename)} is populated although {data($dostxtname)} is populated</error>			
