...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in doc($datasetlocation)[$doseoid and $dostxtoid]//odm:ItemGroupData
        ...
        return <error rule="CG0111" dataset="{data($name)}" variable="{data($dostxtname)}" rulelastupdate="2019-08-11" recordnumber="{data($recnum)}">{data($dostxtname)} is populated although{data($dosename)}  is populated</error>			
