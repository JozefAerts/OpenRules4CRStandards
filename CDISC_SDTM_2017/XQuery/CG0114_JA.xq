...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$doseoid] or odm:ItemData[@ItemOID=$dostxtoid] or odm:ItemData[@ItemOID=$dostotoid]]
        ...
        return <error rule="CG0114" dataset="{data($name)}" variable="{data($dosuname)}" rulelastupdate="2017-02-12" recordnumber="{data($recnum)}">{data($dosuname)} is not populated although one of {data($dosename)}, {data($dostxtname)} or {data($dostotname)} is populated</error>				
