...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or @def:Class='FINDINGS']
    ...
    where not($locoid) and $diroid  (: --LOC is not present, but --DIR is present :)
        return 
            <error rule="CG0116" dataset="{data($name)}" variable="{data($dirname)}" rulelastupdate="2017-02-18">{data($dirname)} is present although {data($locname)} is present in dataset {data($name)}</error>				
