...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmoid]]
        ...
        return <error rule="CG0117" dataset="DM" variable="ACTARM" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for ACTARM, value={data($actarmvalue)} in dataset {data($datasetlocation)}</error>			
