...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        return <error rule="CG0121" dataset="DM" variable="ARM" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}' is not one of ('Screen Failure', 'Not Assigned') and is not found in TA</error>
