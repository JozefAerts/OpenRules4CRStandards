...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        return <error rule="CG0122" dataset="DM" variable="ARM" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}' is expected to be equal to the value of ACTARM='{data($actarmvalue)}'</error>				
