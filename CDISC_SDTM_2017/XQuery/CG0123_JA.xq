...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmcdoid]]
        ...
        return <warning rule="CG0123" dataset="DM" variable="ACTARMCD" ulelastupdate="2019-08-29" recordnumber="{data($recnum)}">Invalid value for ACTARMCD in dataset {data($datasetname)}, it's length is more than 20 characters, value='{data($actarmcdvalue)}'</warning>				
