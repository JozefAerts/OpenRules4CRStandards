...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armcdoid]]
        ...
        return <error rule="CG0128" dataset="DM" variable="ARMCD" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for ARMCD, value='{data($armcdvalue)}' is not found in TA and is not one of('SCRNFAIL', 'NOTASSGN')</error>												
