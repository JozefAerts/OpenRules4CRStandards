...
(: Rule CG0129 - When ARMCD in ('SCRNFAIL', 'NOTASSGN')  then ARMCD = ACTARMCD :)
xquery version "3.0";
...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    (: get the OID of the ARMCD and of ACTARMCD :)
    let $armcdoid := (
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armcdoid]]
        ...
        (: get the value of ARMCD and of ACTARMCD :)
        ...
        where ($armcdvalue='SCRNFAIL' or $armcdvalue='NOTASSGN') and not($armcdvalue=$actarmcdvalue)
        return <error rule="CG0129" dataset="DM" variable="ARMCD" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for ARMCD, value={data($armcdvalue)} is expected to be equal to ACTARMCD={data($actarmcdvalue)}</error>				
