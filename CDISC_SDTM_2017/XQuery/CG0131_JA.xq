...
for $dmitemgroupdef in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in $dmdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$dthfloid]]
        ...
        where not($dthflvalue='Y')
             return <error rule="CG0131" dataset="DM" variable="DTHFL" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">Invalid value for DTHFL, value='{data($dthflvalue)}' is expected to be 'Y' or null</error>			
