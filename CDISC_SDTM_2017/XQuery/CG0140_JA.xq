...
for $dmrecord in $dmdatasetdoc//odm:ItemGroupData
    ...
    return <error rule="CG0140" variable="RACE" dataset="DM" rulelastupdate="2017-02-18" recordnumber="{data($recnum)}">{data($suppdmcountrace)} 'RACE' records for USUBJID='{data($usubjid)}' have been found in SUPPDM, but the value of RACE='{data($race)}' in DM is not 'MULTIPLE'</error>			
