...
for $record in $iedatasetdoc//odm:ItemGroupData
    ...
    return <error rule="CG0177" dataset="IE" variable="IESTRESC" rulelastupdate="2017-02-20" recordnumber="{data($recnum)}">Mismatch between IEORRES and IESTRESC values for USUBJID={data($usubjid)}. IEORRES={data($ieorres)}, whereas IESTRESC={data($iestresc)}</error>			
