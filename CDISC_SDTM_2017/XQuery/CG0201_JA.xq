...
for $record in $relrecdatasetdoc//odm:ItemGroupData[not(odm:ItemData[@ItemOID=$idvarvaloid]) and not(odm:ItemData[@ItemOID=$usubjidoid])]
    ...
    return <error rule="CG0201" dataset="RELREC" variable="IDVAR" recordnumber="{data($recnum)}" rulelastupdate="2017-02-23" >IDVAR={data($idvar)} is not allowed as USUBJID=null and IDVARVAL=null</error>			
