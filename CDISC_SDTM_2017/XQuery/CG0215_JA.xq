...
(: Rule CG0215 - SV - When SVUPDES = null then VISIT in TV.VISIT
...
(: we need the OID of VISIT and of SVUPDES in SV :)
let $svvisitoid := (
...
(: and the OID of VISIT in TV :)
let $tvvisitoid := (
    for $a in $definedoc//odm:ItemDef[@Name='VISIT']/@OID 
        where $a = $tvitemgroupdef/odm:ItemRef/@ItemOID
        return $a
)
(: get all the values of VISIT in TV (as a sequence/array) :)
let $tvvisits := $tvdatasetdoc//odm:ItemGroupData/odm:ItemData[@ItemOID=$tvvisitoid]/@Value
...
for $record in $svdatasetdoc//odm:ItemGroupData[not(odm:ItemData[@ItemOID=$svupdesoid])]
    ...
    let $svvisit := $record/odm:ItemData[@ItemOID=$svvisitoid]/@Value
    (: SV.VISIT must be one of TV.VISIT :)
    where not(functx:is-value-in-sequence($svvisit,$tvvisits))
    return <error rule="CG0215" dataset="SV" variable="VISIT" rulelastupdate="2017-02-24" recordnumber="{data($recnum)}">SV.VISIT='{data($svvisit)}' was not found in TV for SVUPDES=null</error>		
