...
for $datasetdef in $definedoc//odm:ItemGroupDef[not(@Name='DM')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$stdtcoid]]
        ...
        return <error rule="CG0221" dataset="{data($name)}" variable="{data($stdyname)}" rulelastupdate="2017-02-24" recordnumber="{data($recnum)}">{data($stdyname)} variable value {data($stdyvalue)} is not null although one of {data($stdtcname)} (value={data($stdtcvalue)}) or RFSTDTC (value={data($rfstdtcvalue)}) is not a  complete date</error>				
