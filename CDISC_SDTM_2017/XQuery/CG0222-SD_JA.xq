...
for $datasetdef in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='DM') and @Name=$domain]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$endtcoid]]
        ...
        return <error rule="CG0222" dataset="{data($name)}" variable="{data($endyname)}" rulelastupdate="2019-08-12" recordnumber="{data($recnum)}">Invalid value for {data($endyname)}, calculated value={data($endycalculatedfda)}, observed value={data($endyvalue)}, in dataset {data($datasetname)} </error>				
