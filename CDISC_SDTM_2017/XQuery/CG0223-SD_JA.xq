...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='DM')][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$endtcoid]]
        ...
        return <error rule="CG0223" dataset="{data($name)}" variable="{data($endyname)}" rulelastupdate="2010-08-12" recordnumber="{data($recnum)}">{data($endyname)} variable value {data($endyvalue)} is not null although one of {data($endtcname)} (value={data($endtcvalue)}) or RFSTDTC (value={data($rfstdtcvalue)}) is not a  complete date</error>			
