...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[not(@Name='TV')]
    ...
    for $record in $datasetdoc[$visitnumoid and $visitdyoid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$visitnumoid]]
        ...
        return <error rule="CG0225" dataset="{data($name)}" variable="VISITDY" rulelastupdate="2017-02-24" recordnumber="{data($recnum)}">VISITDY={data($visitdy)} is expected to be null as VISITNUM={data($visitnum)} is not present in TV</error>			
