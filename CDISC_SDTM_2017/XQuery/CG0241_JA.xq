...
for $datasetdef in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    for $group in $orderedrecords   
        ...
        where $count != 1
        return <error rule="CG0241" dataset="{data($name)}" variable="{data($eltmname)}" rulelastupdate="2016-02-26">Inconsistent value for {data($eltmname)} within group of DOMAIN/VISITNUM/{data($tptrefname)}/{data($tptnumname)} in dataset {data($datasetname)}. {data($count)} different {data($eltmname)} values were found for this group</error>				
