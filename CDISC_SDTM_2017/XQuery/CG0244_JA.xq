...
for $datasetdef in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TA' or @Name='TV']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        return <error rule="CG0244" dataset="{data($name)}" variable="ARM" recordnumber="{data($recnum)}" rulelastupdate="2017-02-24">Value for planned ARM='{data($arm)}' is not allowed in dataset {data($name)}</error>			
