...
for $record in $tidatasetdoc[$ietestcdoid and not($tiversoid)]//odm:ItemGroupData
    ...
    for $recordnext in $record/following-sibling::odm:ItemGroupData
        ...
        return <error rule="CG0256" dataset="TI" variable="IETESTCD" rulelastupdate="2017-02-25" recordnumber="{data($recnum)}">Records {data($recnum)} and {data($recnumnext)} have the same value for IETESTCD='{data($ietestcd)}' where TIVERS is absent</error>			
