...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0257" dataset="TS" variable="TSPARMCD" recordnumber="{data($recnum)}" rulelastupdate="2017-02-26">The value for TSPARMCD in dataset {data($datasetname)} has more than 8 characters: '{data($testvalue)}' was found</error>			
