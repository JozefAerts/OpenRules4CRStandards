...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsvaloid]]
        ...
        where $tsvalnf 
        return <error rule="CG0260" dataset="TS" variable="TSVALNF" recordnumber="{data($recnum)}" rulelastupdate="2017-02-26">TSVALNF={data($tsvalnf)} is expected to be null as TSVAL={data($tsval)}</error>			
