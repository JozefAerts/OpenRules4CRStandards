    ...
    for $group in $orderedrecords
        ...
        for $record in $group/odm:ItemGroupData[odm:ItemData[@ItemOID=$tsseqoid]]
            ...
            for $recordduplicate in $record/following-sibling::odm:ItemGroupData[odm:ItemData[@ItemOID=$tsseqoid and @Value=$tsseq]]
                ...
                return <error rule="CG0268" dataset="{data($name)}" variable="TSSEQ" rulelastupdate="2017-02-26">Inconsistent value for TSSEQ within TSPARMCD={data($tsparmcd)} in dataset {data($datasetname)}. Records {data($recnum)} and {data($recnumduplicate)} have the same value for TSSEQ={data($tsseq)}</error>			
