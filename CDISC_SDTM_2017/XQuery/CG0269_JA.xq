...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsvaloid] and odm:ItemData[@ItemOID=$tsparmcdoid and @Value='ADDON']]
        ...
        return <error rule="CG0269" dataset="TS" variable="TSVAL" recordnumber="{data($recnum)}" rulelastupdate="2019-08-29">TSVAL='{data($tsval)}' for TSPARMCD='ADDON' is expected to be in ('N','Y')</error>			
