...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    (: TSPARMCD = 'TDIGRP' must not be null :)
    where $nothltsubjects and (not($tdigrpvalue))  (: give an error when TSVAL for TSPARMCD=TDIGRP is null  :)
    return <error rule="CG0273" dataset="TS" variable="TSVAL" rulelastupdate="2017-02-26">Null value for TSVAL for TSPARMCD='TDIGRP' found although a record with TSPARMCD='HLTSUBJI and TSVAL='N' exists</error>			
