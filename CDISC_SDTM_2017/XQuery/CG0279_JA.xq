...
(: Rule CG0279 - When TSPARMCD = 'PCLAS' and record exists where TSPARMCD = 'STYPE' and TSVAL = 'INTERVENTIONAL' and Pharmacological Class of  Interventional Therapy is applicable then TSVAL ^= null
 :)
...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    (: look for a record with TSPARMCD='PCLAS' :)
    let $trtrecord :=  $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='PCLAS']]
    ...
    return <error rule="CG0279" dataset="TS" variable="TSVAL" rulelastupdate="2017-02-27">Null value for TSVAL for TSPARMCD='PCLAS' found although a record with TSPARMCD='STYPE and TSVAL='INTERVENTIONAL' exists</error>			
