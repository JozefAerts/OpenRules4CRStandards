...
(: Rule CG0283 - When TSPARMCD = DCUTDTC then TSVAL conforms to ISO 8601 date :)
xquery version "3.0";
...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    (: look for a record with TSPARMCD='DCUTDTC' - there should be only 1 :)
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='DCUTDTC']]
        let $recnum := $record/@data:ItemGroupDataSeq
        (: get the value of TSVAL in this record (if any) :)
        let $tsval := $record/odm:ItemData[@ItemOID=$tsvaloid]/@Value
        (: TSVAL must conform to ISO 8601 date :)
        where $tsval and not($tsval castable as xs:date)
        return <error rule="CG0283" dataset="TS" variable="TSVAL" record="{data($recnum)}" rulelastupdate="2017-02-27">TSVAL={data($tsval)} for TSPARMCD=DCUTDTC is expected to be a valid ISO-8601 date</error>				
