...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='SENDTC']]
        ...
        return <error rule="CG0286" dataset="TS" variable="TSVAL" record="{data($recnum)}" rulelastupdate="2017-02-27">TSVAL={data($tsval)} for TSPARMCD=SENDTC is expected to be a valid ISO-8601 date</error>				
