...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $itemoid in $itemgroupdef/odm:ItemRef/@ItemOID
        ...
        return <error rule="CG0303" dataset="{data($name)}" variable="{data($varname)}" rulelastupdate="2019-08-22">Label '{data($label)}' does not correspond to the expected label from the SDTM-IG '{data($labelfromwebservice)}' </error>			
