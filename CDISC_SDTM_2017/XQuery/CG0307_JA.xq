...
for $datasetdef in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $tsparmcduniquevalue in $tsparmcduniquevalues
        ...
        return <error rule="CG0307" dataset="{data($name)}" variable="TSPARMCD" rulelastupdate="2017-03-06">Inconsistent value for TSPARM within TSPARMCD in dataset {data($datasetname)}. {data($uniquetsparmvaluescount)} different TSPARM values were found for TSPARMCD={data($tsparmcduniquevalue)}</error>				
