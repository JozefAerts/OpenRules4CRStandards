...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[not(@Name='AP') and not(@Name='RELREC') and not(@Name='SUPPAP')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$domainoid]]
        ...
        return <error rule="CG0308" dataset="{data($name)}" variable="DOMAIN" recordnumber="{data($recnum)}" rulelastupdate="2017-03-06">The value for DOMAIN in the dataset does not consist of exactly 2 characters: DOMAIN='{data($domainvalue)}' was found</error>							
