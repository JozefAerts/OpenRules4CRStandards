...
for $record in $tedatasetdoc//odm:ItemGroupData
    ...
    for $record2 in $record/following-sibling::odm:ItemGroupData[odm:ItemData[@ItemOID=$testrloid and @Value=$testrl]][odm:ItemData[@ItemOID=$teenrloid and @Value=$teenrl]]
        ...
        return <error rule="CG0325" dataset="TE" recordnumber="{data($recnum2)}" rulelastupdate="2017-03-11">Record {data($recnum2)} with ETCD='{data($etcd2)}' has the same combination of values of  TESTRL='{data($testrl)}' and TEENRL='{data($teenrl)}' as record {data($recnum)} with ETCD='{data($etcd)}'</error>											
