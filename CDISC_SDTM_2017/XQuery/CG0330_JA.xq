...
for $dataset in $datasetdoc//odm:ItemGroupDef
    ...
    return <error rule="CDG0330" dataset="{data($datasetname)}" rulelastupdate="2017-03-11">{data($datasetname)} - domain {data($domaincode)}: Invalid order of variables in define.xml - found: {data($vararray)}
