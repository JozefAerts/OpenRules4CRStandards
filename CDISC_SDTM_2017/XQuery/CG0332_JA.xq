...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[(string-length(@Name)=3 or string-length(@Name)=4) and not(starts-with(@Name,'AP')) and not(starts-with(@Name,'FA'))]
    ...
    return <error rule="CG0332" dataset="{data($name)}" rulelastupdate="2017-03-11">First two characters of dataset name '{data($name)}' does not correspond to the domain name '{data($domain)}'</error>			
