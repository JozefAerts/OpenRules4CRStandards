...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$catoid]]
        ...
        where $cat=$domain
        return <error rule="CG0336" dataset="{data($name)}" variable="{data($catname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($catname)}='{data($cat)}' is not allowed to be equal to the name of the domain '{data($domain)}'</error>			
