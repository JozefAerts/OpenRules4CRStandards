...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$catoid] and odm:ItemData[@ItemOID=$decodoid]]
        ...
        where $cat=$decod
        return <error rule="CG0337" dataset="{data($name)}" variable="{data($catname)}" recordnumber="{data($recnum)}" rulelastupdate="2017-03-11">Value of {data($catname)}='{data($cat)}' is not allowed to be equal to the value of {data($decodname)}='{data($decod)}'</error>			
