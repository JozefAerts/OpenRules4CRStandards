...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname][upper-case(@def:Class)='FINDINGS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$testcdoid]
        ...
        return <error rule="CG0341" dataset="{data($name)}" variable="{data($testcdname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($testcdname)}='{data($testcd)}' is not allowed to be equal to 'OTHER'</error>						
