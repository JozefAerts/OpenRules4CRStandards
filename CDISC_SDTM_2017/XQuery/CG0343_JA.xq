...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='EVENTS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$termoid]
        ...
        return <error rule="CG0343" dataset="{data($name)}" variable="{data($termname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($termname)}='{data($term)}' is not allowed to be equal to 'OTHER'</error>						
