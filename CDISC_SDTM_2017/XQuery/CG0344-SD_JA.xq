...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS'][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$testcdoid]
        ...
        return <error rule="CG0344" dataset="{data($name)}" variable="{data($testcdname)}" recordnumber="{data($recnum)}" rulelastupdate="2017-03-14">Value of {data($testcdname)}='{data($testcd)}' is not allowed to be equal to 'MULTIPLE'</error>									
