...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$trtoid]
        ...
        return <error rule="CG0345" dataset="{data($name)}" variable="{data($trtname)}" recordnumber="{data($recnum)}" rulelastupdate="2017-03-14">Value of {data($trtname)}='{data($trt)}' is not allowed to be equal to 'MULTIPLE'</error>									
