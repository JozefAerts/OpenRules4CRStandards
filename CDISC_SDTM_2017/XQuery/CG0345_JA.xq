...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$trtoid]
        ...
        return <error rule="CG0345" dataset="{data($name)}" variable="{data($trtname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($trtname)}='{data($trt)}' is not allowed to be equal to 'MULTIPLE'</error>									
