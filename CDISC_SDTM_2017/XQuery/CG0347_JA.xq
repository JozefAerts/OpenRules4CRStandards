...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$orresoid]
        ...
        return <error rule="CG0347" dataset="{data($name)}" variable="{data($orresname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($orresname)}='{data($orres)}' is not allowed to be equal to 'MULTIPLE'</error>												
