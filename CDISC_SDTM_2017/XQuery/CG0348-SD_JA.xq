...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS'][@Name=$datasetname]
    ...
    for $record in $datasetdoc[$orresoid and ($statoid or $drvfloid)]//odm:ItemGroupData[$statoid and (not(odm:ItemData[@ItemOID=$statoid])) or ($drvfloid and not(odm:ItemData[@ItemOID=$drvfloid]/@Value='Y'))]
        ...
        where not($orres)
        return <error rule="CG0348" dataset="{data($name)}" variable="{data($orresname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value of {data($orresname)} may not be null when {data($statname)}=NULL or {data($drvflname)}!='Y'</error>												
