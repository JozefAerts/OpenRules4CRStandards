...
for $apitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AP')]
    ...
    for $record in $apdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$rsubjidoid]]
        ...
        (: RDEVID must be null :)
        where $rdevid
        return <error rule="CG0365" dataset="{data($name)}" variable="RDEVID" recordnumber="{data($recnum)}" rulelastupdate="2017-03-15">RDEVID='{data($rdevid)}' must be null as RSUBJID='{data($rsubjid)}' is not null</error>						
