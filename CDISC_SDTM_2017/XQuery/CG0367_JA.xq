...
for $apitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AP')]
    ...
    for $record in $apdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$rsubjidoid] and odm:ItemData[@ItemOID=$poolidoid] 
        ...
        return <error rule="CG0367" dataset="{data($name)}" variable="RSUBJID" recordnumber="{data($recnum)}" rulelastupdate="2017-03-15">Value of RSUBJID='{data($rsubjid)}' was not found as USUBJID in DM dataset although RSUBJID != POOLID='{data($poolid)}'</error>			
