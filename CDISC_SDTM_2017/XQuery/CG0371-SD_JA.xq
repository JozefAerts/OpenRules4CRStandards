...
for $dataset in $datasets
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0371" dataset="{data($name)}" variable="IDVARVAL" rulelastupdate="2017-03-15" recordnumber="{data($recnum)}">Invalid referenced record - Dataset={data($name)}: no parent records found in RDOMAIN={data($rdomainval)} for USUBJID={data($usubjidvalue)} with IDVAR='{data($idvarvalue)}' and IDVARVAL='{data($idvarvalvalue)}'</error>			
