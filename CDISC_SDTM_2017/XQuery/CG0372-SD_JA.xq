...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $record in doc($dataset)//odm:ItemGroupData[odm:ItemData[@ItemOID=$testoid]]
        ...
        return <error rule="CG0372" recordnumber="{data($recnum)}" dataset="{data($itemgroupdefname)}" variable="{$testname}" rulelastupdate="2019-08-12">{data($testname)}='{data($value)}' must be not more than 8 chars long and contains only letters, numbers, and underscores and can not start with a number</error>			
