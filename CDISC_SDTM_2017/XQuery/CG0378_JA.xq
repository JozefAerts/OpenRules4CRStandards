...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0378" dataset="{data($name)}" variable="{data($lltcdname)}" recordnumber="{data($recnum)}" rulelastupdate="2017-03-16">Value '{data($lltcdvalue)}' for {data($lltcdname)} was not found in the MedDRA dictionary</error>						
