...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0380" dataset="{data($name)}" variable="{data($ptcdname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value '{data($ptcdvalue)}' for {data($ptcdname)} was not found in the MedDRA dictionary as a preferred term code</error>			
