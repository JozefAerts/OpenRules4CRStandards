...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0386" dataset="{data($name)}" variable="{data($bdsycdname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value '{data($bdsycdvalue)}' for {data($bdsycdname)} is not found in the MedDRA dictionary</error>
