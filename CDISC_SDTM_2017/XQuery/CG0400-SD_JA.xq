    ...
    for $group in $orderedrecords
        ...
        let $errormessages := 
            if($webserviceresponse='null' or string-length($webserviceresponse)=0) then
                ...
                return <error rule="CG0400" dataset="{data($name)}" recordnumber="{data($recnum)}" variable="{data($loincname)}" rulelastupdate="2017-03-17">LOINC code for {data($loincname)}='{data($loinc)}' is not recognized as a valid LOINC code by the XML4Pharma RESTful web service</error> 
            else ()  
