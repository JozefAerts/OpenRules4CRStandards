...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS'][not(starts-with(@Name,'IE'))][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0406" dataset="{data($name)}" variable="{data($testname)}" rulelastupdate="2019-08-12">The value of {data($testname)} has more than 40 characters. {data($testname)}='{data($test)}' was found</error>												
