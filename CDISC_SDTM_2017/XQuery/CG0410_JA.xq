...
(: Rule CG0410 - VISITNUM identifies unique record within subject :)
...
for $group in $orderedrecords
    ...
    where $count > 1
    return <error rule="CG0410" dataset="SV" rulelastupdate="2017-03-19" recordnumber="{data($recnum)}">SV.VISITNUM does not uniquely identify records within subject - combination of USUBJID={data($usubjidvalue)} and VISITNUM={data($visitnumvalue)} occurrs {data($count)} times</error>				
