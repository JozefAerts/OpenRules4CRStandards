...
(: Rule CG0411 - IDVAR, IDVARVAL, and QNAM a unique combination per parent subject record :)
...
for $group in $orderedrecords
    ...
    where $count > 1
    return <error rule="CG0411" dataset="{data($name)}" rulelastupdate="2017-03-19" recordnumber="{data($recnum)}">The combination of USUBJID={data($usubjidvalue)}, IDVAR={data($idvar)}, IDVARVAL={data($idvarval)} and QNAM={data($qnam)} is not unique</error>			
