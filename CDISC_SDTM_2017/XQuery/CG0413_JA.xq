...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[not(starts-with(@Name,'SUPP'))]
    ...
    return <error rule="CG0413" dataset="{data($name)}" rulelastupdate="2017-03-22" >Dataset name='{data($name)}' or file name='{data($datasetfilename)}' does not start with the domain name='{data($domain)}'</error>						
