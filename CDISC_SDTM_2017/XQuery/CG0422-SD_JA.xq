...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS'][@Domain=$datasetname or starts-with(@Name,$datasetname)]
    ...
    for $record in $datasetdoc[$orresoid and $statoid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$orresoid]/@Value]
        ...
        (: STAT must be null :)
        where $statvalue
        return <error rule="CG0422" dataset="{data($name)}" variable="{data($statname)}" rulelastupdate="2019-08-13" recordnumber="{data($recnum)}">{data($statname)} must be NULL, as ({data($orresname)})='{data($orresvalue)}' is not null. {data($statname)}='{data($statvalue)}' is found</error>
