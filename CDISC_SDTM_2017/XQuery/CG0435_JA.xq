...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$dhdtcoid]]
        ...
        return <error rule="CG0435" dataset="DM" variable="DTHFL" rulelastupdate="2017-03-22" recordnumber="{data($recnum)}">DTHFL='{data($dthflvalue)}' instead of 'Y', although DTHDTC='{data($dthflvalue)}' is not null</error>	
