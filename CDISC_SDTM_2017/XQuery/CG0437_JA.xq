...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0437" dataset="{data($name)}" variable="{data($soccdname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-13">Value '{data($soccdvalue)}' for {data($soccdname)} was not found in the MedDRA dictionary</error>						
