...
(: When TSVAL!=null then TSVAK is in ISO-8601 "duration" format :)
where $tsvalvalue and not($tsvalvalue castable as xs:duration)
return <error rule="CG0438" dataset="TS" variable="TSVAL" rulelastupdate="2017-03-23" recordnumber="{data($tsageminrecordnumber)}">Invalid (non-ISO8601) TSVAL value={data($tsvalvalue)} for AGEMIN in dataset TS</error>				
