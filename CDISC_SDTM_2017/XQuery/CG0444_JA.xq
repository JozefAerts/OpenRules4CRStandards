...
for $record in doc($tsdatasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and (@Value='CURTRT' or @Value='TRT' or @Value='COMPTRT')]]
    ...
    return <error rule="CG0444" dataset="TS" variable="TSVCDREF" rulelastupdate="2017-03-23" recordnumber="{data($recnum)}">Invalid TSVCDREF value='{data($tsvcdrefvalue)}' for TSPARMCD={data($tsparmcdvalue)} in dataset TS. The value must be 'UNII'</error>			
