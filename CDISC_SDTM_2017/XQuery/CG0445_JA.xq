...
for $record in doc($tsdatasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid]][1]
    ...
    :)
    where $uniicount=0
    return <error rule="CG0445" dataset="TS" variable="TSVAL" rulelastupdate="2017-03-23" recordnumber="{data($recnum)}">Invalid TSVAL value={data($tsvalvalue)} for TSPARMCD=COMPTRT in dataset TS</error>
