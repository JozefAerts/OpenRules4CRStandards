...
for $record in $tsdataset//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='PCLAS']][odm:ItemData[@ItemOID=$tsvaloid]]
    ...
    where $count = 0
    return <error rule="CG0451" dataset="TS" variable="TSVALCD" rulelastupdate="2017-03-24" recordnumber="{data($recnum)}">Invalid TSVAL value for PCLASS in dataset {data($tsdatasetname)}: TSVAL for TSPARMCD=PCLAS record must be a valid a valid term from NDF-RT: value '{data($tsval)}' was found</error> 			
