...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='EVENTS' and not(functx:is-value-in-sequence(substring(@Name,1,2),$eventsdomains))]
    ...
    where not($termoid)
    return <error rule="CG0464" dataset="{data($name)}" variable="{data($varname)}" rulelastupdate="2017-03-25">{data($varname)} is not present in EVENTS custom domain {data($name)}</error>										
