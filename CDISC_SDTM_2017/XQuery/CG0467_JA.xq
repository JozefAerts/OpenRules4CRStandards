...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS']
    ...
    where $stdtcoid
    return <error rule="CG0467" dataset="{data($name)}" variable="" rulelastupdate="2017-03-25">Variable {data($stdtcname)} is not allowed to be present in the FINDINGS dataset {data($name)}</error>													
