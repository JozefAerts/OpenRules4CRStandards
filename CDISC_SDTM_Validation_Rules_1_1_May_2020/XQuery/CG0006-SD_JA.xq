...
for $dataset in $definedoc//odm:ItemGroupDef[@Name=$datasetname and not(@Name='DM')]
	...
	let $datasetdoc := (
		if($datasetname) then doc(concat($base,$dsname))
		else ()
	)
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$dtcoid]]
        ...
        return <error rule="CG0006" dataset="{data($name)}" variable="{data($dyname)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid value for {data($dyname)}, calculated value={data($dycalculatedfda)}, observed value={data($dyvalue)}, in dataset {data($name)} </error>	
