...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='DM')]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$dtcoid]]
        ...
        return <error rule="CG0006" dataset="{data($name)}" variable="{data($dyname)}" rulelastupdate="2020-06-10" recordnumber="{data($recnum)}">Invalid value for {data($dyname)}, calculated value={data($dycalculatedfda)}, observed value={data($dyvalue)}, in dataset {data($name)} </error>	
