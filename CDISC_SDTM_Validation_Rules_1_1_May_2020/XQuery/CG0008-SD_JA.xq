...
for $dataset in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$eltmoid]]
        ...
        return <error rule="CG0008" variable="{data($tptrefname)}" dataset="{data($name)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Missing value for {data($eltmname)} is not null although {data($tptrefname)} is not populated, value of {data($eltmname)}={data($eltmvalue)}</error>						
