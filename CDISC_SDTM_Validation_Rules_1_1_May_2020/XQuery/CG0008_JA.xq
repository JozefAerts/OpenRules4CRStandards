...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='DM')]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$eltmoid]]
        ...
        return <error rule="CG0008" variable="{data($tptrefname)}" dataset="{data($name)}" rulelastupdate="2020-06-08" recordnumber="{data($recnum)}">Missing value for {data($eltmname)} is not null although {data($tptrefname)} is not populated, value of {data($eltmname)}={data($eltmvalue)}</error>				
