...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname or @Domain=$datasetname]
    let $dataset := $itemgroupdef/def:leaf/@xlink:href
    let $dsname := $itemgroupdef/@Name
    ...
    for $d in doc($datasetpath)//odm:ItemGroupData[odm:ItemData[@ItemOID=$usubjidoid]]
        ...
        return <error rule="CG0029" rulelastupdate="2017-02-04" dataset="{data($dsname)}" variable="USUBJID"  recordnumber="{data($recnum)}">USUBJID {data($value)} in dataset {data($datasetname)} could not be found in DM dataset</error>
