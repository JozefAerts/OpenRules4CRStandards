...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[not(starts-with(@Name,'AP'))]
    ...
    for $d in doc($datasetpath)//odm:ItemGroupData[odm:ItemData[@ItemOID=$usubjidoid]]
        ...
        return <error rule="CG0029" rulelastupdate="2020-06-11" dataset="{data($datasetname)}" variable="USUBJID"  recordnumber="{data($recnum)}">USUBJID {data($value)} in dataset {data($datasetname)} could not be found in DM dataset</error>
