...
(: Rule CG0029 FAST ALTERNATIVE - All Subjects (USUBJID) must be present in Demographics (DM) domain :)
xquery version "3.0";
...
(: except for AP datasets (associated persons), but these do ot have USUBJID anyway :)
for $itemgroupdef in $definedoc//odm:ItemGroupDef[not(starts-with(@Name,'AP'))]
	let $dataset := (
    ...
    for $d in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$usubjidoid]]
        ...
        return <error rule="CG0029" rulelastupdate="2020-08-04" dataset="{data($datasetname)}" variable="USUBJID"  recordnumber="{data($recnum)}">USUBJID {data($value)} in dataset {data($datasetname)} could not be found in DM dataset</error>
