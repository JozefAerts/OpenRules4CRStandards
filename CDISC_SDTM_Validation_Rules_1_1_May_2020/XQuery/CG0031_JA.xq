...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='TV')]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$visitnumoid] and odm:ItemData[@ItemOID=$visitoid]]
        ...
        return <error rule="CG0031" dataset="{data($name)}" variable="VISITNUM" rulelastupdate="2020-06-09" recordnumber="{data($recnum)}">VISIT={data($visitvalue)}, VISITNUM={data($visitnumvalue)} in dataset {data($datasetname)} is not defined in the TV dataset</error>			
