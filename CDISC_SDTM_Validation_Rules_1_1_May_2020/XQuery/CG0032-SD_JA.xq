...
for $dataset in $definedoc//odm:ItemGroupDef[@Name=$datasetname and not(@Name='TV')]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$visitnumoid] and odm:ItemData[@ItemOID=$visitdyoid]]
        ...
        return <error rule="CG0032" dataset="{data($name)}" variable="VISITDY" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">VISITDY={data($visitdyvalue)} for VISITNUM={data($visitnumvalue)} in dataset {data($datasetname)} is not found in the TV dataset</error>			
