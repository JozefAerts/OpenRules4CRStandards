...
for $dataset in $definedoc//odm:ItemGroupDef[not(@Name='TV')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$visitnumoid]]
        ...
        return <error rule="CG0033" dataset="{data($name)}" variable="VISITNUM" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Planned VISITNUM={data($visitnumvalue)} in dataset {data($datasetname)} is not found in the TV dataset</error>
