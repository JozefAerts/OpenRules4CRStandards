...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS'  
    ...
    for $record in $datasetdoc//odm:ItemGroupData[$bodsysoid and $socoid]
        ...
        return <error rule="CG0039" dataset="{data($name)}" variable="{data($bodsysname)}" rulelastupdate="2020-06-09" recordnumber="{data($recnum)}">Value of {data($bodsysname)} '{data($bodsysvalue)}' and {data($socname)} '{data($socvalue)}' may not differ</error>	
