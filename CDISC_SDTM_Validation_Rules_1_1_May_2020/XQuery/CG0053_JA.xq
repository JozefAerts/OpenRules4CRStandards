...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS'][not(@Domain='DS') and not(@Domain='DV') and not(@Domain='EX')
    ...
    (: When --PRESP not present then --REASND not present in dataset  :)
    where not($prespoid) and $reasndoid 
    return <error rule="CG0053" variable="{data($reasndname)}" dataset="{$name}" rulelastupdate="2020-08-04">{data($prespname)} is present although {data($reasndname)} is absent</error>			
