...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS'][not(@Domain='DS') and not(@Domain='DV') and not(@Domain='EX')]
	...
	(: 2017: where not($reasndoid) and $prespoid :)
    return <error rule="CG0053" variable="{data($reasndname)}" dataset="{$name}" rulelastupdate="2020-06-09">{data($prespname)} is present although {data($reasndname)} is absent</error>			
