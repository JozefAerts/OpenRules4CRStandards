...
(: Applies to: EVENTS,INTERVENTIONS, NOT(DS, DV, EX) :)
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS' or upper-case(./def21:Class/@Name)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='EVENTS'][not(@Name='DS') and not(@Name='DV') and not(@Name='EX')][@Name=$datasetname]
    ...
    return <error rule="CG0081" variable="{data($statname)}" dataset="{$name}" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">Invalid value for {data($statname)}='{data($stat)}' for {data($prespname)}='Y' and {data($occurname)}=null. Value for {data($statname)} expected is 'NOT DONE'</error>				
