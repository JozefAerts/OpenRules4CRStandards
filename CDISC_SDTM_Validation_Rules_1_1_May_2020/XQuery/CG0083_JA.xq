...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='EVENTS' or upper-case(@def:Class)='INTERVENTIONS']
    ...
    for $group in $orderedrecords[$bdsycdoid and $bodsysoid]
        ...
        for $record in $group/odm:ItemGroupData[position()>1]
            ...
            return <error rule="CG0083" rulelastupdate="2020-06-11" dataset="{data($name)}" variable="{data($bdsycdname)}" recordnumber="{data($recnum2)}">Combination of {data($bdsycdname)} and {data($bodsysname)} in dataset {data($datasetname)} is inconsistent: 
