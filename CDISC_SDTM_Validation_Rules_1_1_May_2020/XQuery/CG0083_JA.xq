...
(: Rule CG0083 - --BDSYCD and --BODSYS have a one-to-one relationship 
 Scope: EVENTS and INTERVENTIONS datasets :)
xquery version "3.0";
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='EVENTS' or upper-case(@def:Class)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='EVENTS']
    ...
    for $group in $orderedrecords[$bdsycdoid and $bodsysoid]
        ...
        for $record in $group/odm:ItemGroupData[position()>1]
            ...
            return <error rule="CG0083" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($bdsycdname)}" recordnumber="{data($recnum2)}">Combination of {data($bdsycdname)} and {data($bodsysname)} in dataset {data($datasetname)} is inconsistent: 
