...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'LB') or starts-with(@Name,'AE')]
    ...
    return 
    <error rule="CG0084" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($toxname)}">{data($toxname)} is present, although {data($toxgrname)} is absent in dataset {data($name)}</error>	
