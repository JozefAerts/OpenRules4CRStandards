...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')][@Name=$datasetname]
    ...
    for $record in $datasetdoc[$statoid and $occuroid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$prespoid and @Value='Y'] and not(odm:ItemData[@ItemOID=$statoid])]
        ...
        return
            <error rule="CG0086" rulelastupdate="2020-06-11" dataset="{data($name)}" variable="{data($occurname)}" recordnumber="{data($recnum)}">{data($occurname)}=null for record with {data($prespname)}='Y' and {data($statname)}=null</error>	
