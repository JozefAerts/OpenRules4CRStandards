...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS' or upper-case(./def21:Class/@Name)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')][@Name=$datasetname]
    (: get the dataset :)
    ...
    for $record in $datasetdoc[$statoid and $occuroid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$prespoid and not(@Value='Y')] and odm:ItemData[@ItemOID=$statoid]/@Value='NOT DONE']
        ...
        return
            <error rule="CG0087" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($occurname)}" recordnumber="{data($recnum)}">{data($occurname)}='{data($occur)}' for record with {data($prespname)}='{data($presp)}' and {data($statname)}='NOT DONE'. {data($occurname)} should be null</error>			
