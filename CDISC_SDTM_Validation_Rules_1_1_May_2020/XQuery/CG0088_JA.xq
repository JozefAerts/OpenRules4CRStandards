...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@def:Class='INTERVENTIONS' or @def:Class='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')]
    ...
    return 
         <error rule="CG0088" rulelastupdate="2020-06-11" dataset="{data($name)}" variable="{data($occurname)}" >Variable {data($occurname)} is present in dataset {data($name)} although {data($prespname)} is not present in the dataset</error>			
