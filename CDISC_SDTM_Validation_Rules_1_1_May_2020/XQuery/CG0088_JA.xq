...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS' or upper-case(./def21:Class/@Name)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')]
    ...
    return 
         <error rule="CG0088" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($occurname)}" >Variable {data($occurname)} is present in dataset {data($name)} although {data($prespname)} is not present in the dataset</error>			
