...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS' or upper-case(./def21:Class/@Name)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='EVENTS' and not(@Name='AE') and not(@Name='DS') and not(@Name='DV')][@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$occuroid and @Value!='']]
        ...
        where not($presp='Y')
        return
            <error rule="CG0089" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($prespname)}" recordnumber="{data($recnum)}">{data($prespname)}='{data($presp)}' found for {data($occurname)}='{data($occur)}'. Value expected for {data($prespname)}='Y'</error>			
