...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    return
        <error rule="CG0090" rulelastupdate="2020-06-11" dataset="{data($name)}" variable="{data($tptrefname)}" >Variable {data($rftdtcname)} is present in dataset {data($name)} but {data($tptrefname)} is not present in the dataset</error>				
