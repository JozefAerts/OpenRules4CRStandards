...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    return
        <error rule="CG0092" rulelastupdate="2020-08-04" dataset="{data($name)}" variable="{data($tptrefname)}" >Variable {data($eltmname)} is present in dataset {data($name)} but {data($tptrefname)} is not present in the dataset</error>				
