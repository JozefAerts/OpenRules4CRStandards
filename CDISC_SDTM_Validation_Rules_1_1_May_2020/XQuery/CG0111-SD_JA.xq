...
for $dataset in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in doc($datasetlocation)[$doseoid and $dostxtoid]//odm:ItemGroupData
        ...
        return <error rule="CG0111" dataset="{data($name)}" variable="{data($dostxtname)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">{data($dostxtname)} is populated although{data($dosename)}  is populated</error>			
