...
for $dataset in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='INTERVENTIONS'][@Name=$datasetname]
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData
        ...
        return <error rule="CG0112" dataset="{data($name)}" variable="{data($dostxtname)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">{data($dostxtname)} may not be a numeric value. {data($dostxtname)} = '{data($dostxtvalue)}' was found</error>				
