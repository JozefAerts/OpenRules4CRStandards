...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData
        ...
        return <error rule="CG0112" dataset="{data($name)}" variable="{data($dostxtname)}" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">{data($dostxtname)} may not be a numeric value. {data($dostxtname)} = '{data($dostxtvalue)}' was found</error>				
