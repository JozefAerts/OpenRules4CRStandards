...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmoid]]
        ...
        return <error rule="CG0119" dataset="DM" variable="ACTARM" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}', expected value='{data($actarmvalue)}' </error>				
