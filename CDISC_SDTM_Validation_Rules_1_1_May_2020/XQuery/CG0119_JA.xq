...
(: Rule CG0119 does NOT apply to SDTMIG v.3.3, only to v.3.2 :)
xquery version "3.0";
...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmoid]]
        ...
        return <error rule="CG0119" dataset="DM" variable="ACTARM" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}', expected value='{data($actarmvalue)}' </error>				
