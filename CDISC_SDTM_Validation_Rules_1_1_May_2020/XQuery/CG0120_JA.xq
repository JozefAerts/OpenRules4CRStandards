...
(: Rule CG0120 does NOT apply to SDTMIG v.3.3, only to v.3.2 :)
xquery version "3.0";
...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        return <error rule="CG0120" dataset="DM" variable="ARM" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}' is not in TA and is not one of ('Screen Failure', 'Not Assigned')</error>		
