...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        return <error rule="CG0120" dataset="DM" variable="ARM" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}' is not in TA and is not one of ('Screen Failure', 'Not Assigned')</error>		
