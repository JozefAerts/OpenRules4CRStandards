...
(: Rule CG0121 - When ARM not in ('Screen Failure', 'Not Assigned') then ARM in TA.ARM :)
(: Rule CG0121 does NOT apply to SDTMIG v.3.3, only to v.3.2 :)
xquery version "3.0";
...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid]]
        ...
        where not($armvalue='Screen Failure') and not($armvalue='Not Assigned') and not(functx:is-value-in-sequence($armvalue,$armsequence))
        return <error rule="CG0121" dataset="DM" variable="ARM" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ARM, value='{data($armvalue)}' is not one of ('Screen Failure', 'Not Assigned') and is not found in TA</error>
