...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmcdoid]]
        ...
        return <error rule="CG0124" dataset="DM" variable="ACTARMCD" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid value for ACTARMCD, value={data($actarmcdvalue)} in dataset DM is not found in TA.ARMCD and is not one of ('SCRNFAIL', 'NOTASSGN', 'NOTTRT', 'UNPLAN')</error>				
