...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmcdoid]]
        ...
        return <error rule="CG0125" dataset="DM" variable="ACTARMCD" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ACTARMCD, value={data($actarmcdvalue)} in dataset DM is not one of ('SCRNFAIL', 'NOTASSGN', 'NOTTRT', 'UNPLAN') and is not found in TA.ARMCD</error>			
