...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armcdoid]]
        ...
        return <error rule="CG0126" dataset="DM" variable="ARMCD" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid value for ARMCD, value={data($armcdvalue)} in dataset DM is not one of ('SCRNFAIL', 'NOTASSGN') and is not found in TA.ARMCD</error>			
