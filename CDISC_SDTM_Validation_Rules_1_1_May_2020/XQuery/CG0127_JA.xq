...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armcdoid]]
        ...
        return <error rule="CG0127" dataset="DM" variable="ACTARMCD" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ACTARMCD, value='{data($actarmcdvalue)}' is expected to be equal to the value of ARMCD='{data($armcdvalue)}'</error>			
