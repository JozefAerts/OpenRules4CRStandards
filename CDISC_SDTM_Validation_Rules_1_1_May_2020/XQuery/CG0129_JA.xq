...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$armcdoid]]
        ...
        return <error rule="CG0129" dataset="DM" variable="ARMCD" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">Invalid value for ARMCD, value={data($armcdvalue)} is expected to be equal to ACTARMCD={data($actarmcdvalue)}</error>				
