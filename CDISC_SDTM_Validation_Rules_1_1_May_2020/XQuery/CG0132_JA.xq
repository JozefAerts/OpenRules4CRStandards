...
for $record in $ssdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$ssstrescoid and @Value='DEAD']]
    ...
    where not($dthfl='Y')
    return <error rule="CG0132" variable="DTHFL" dataset="DM" rulelastupdate="2020-08-04">A record (record number={data($recnumss)}) for USUBJID='{data($ssusubjid)}' has been found for which SSSTRESC='DEAD' but the value of DTHFL='{data($dthfl)}' in DM is not 'Y'</error>			
