...
for $subject in $deadsubjects
    ...
    (: DTHFL must be 'Y' :)
    where not($dthfl='Y')
    return <error rule="CG0133" variable="DTHFL" dataset="DM" rulelastupdate="2020-06-14">A record for USUBJID='{data($subject)}' has been found in the DD dataset but the value of DTHFL='{data($dthfl)}' in DM is not 'Y'</error>						
