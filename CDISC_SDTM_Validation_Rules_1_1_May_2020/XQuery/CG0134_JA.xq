...
(: iterate over all records in AE for which AEOUT='FATAL' :)
    for $record in $aedatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aeoutoid and @Value='FATAL']]
        ...
        (: DTHFL must be 'Y' :)
        where not($dthfl='Y')
        return <error rule="CG0134" variable="DTHFL" dataset="DM" rulelastupdate="2020-08-04">A record for USUBJID='{data($usubjid)}' (record number {data($recnum)}) has been found in the AE dataset with AEOUT='FATAL' but the value of DTHFL='{data($dthfl)}' in DM is not 'Y'</error>		
