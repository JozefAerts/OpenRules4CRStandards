...
(: Rule CG0135 - When AE.AESDTH = 'Y' then DTHFL = 'Y' :)
xquery version "3.0";
...
let $aedatasetlocation := $aeitemgroupdef/def:leaf/@xlink:href
...
(: iterate over all records in AE for which AESDTH='Y' :)
    for $record in $aedatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aesdthoid and @Value='Y']]
        ...
        (: DTHFL must be 'Y' :)
        where not($dthfl='Y')
        return <error rule="CG0135" variable="DTHFL" dataset="DM" rulelastupdate="2020-06-14">A record for USUBJID='{data($usubjid)}' (record number {data($recnum)}) has been found in the AE dataset with AESDTH='Y' but the value of DTHFL='{data($dthfl)}' in DM is not 'Y'</error>				
