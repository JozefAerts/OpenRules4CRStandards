...
(: Rule CG0145 - When Milestone associated with RFSTDTC is start of treatment and ACTARM in ('Screen Failure' 'Not Assigned' 'Not Treated') then RFSTDTC = null  :)
(: N/A for v3.3 (new variable ARMNRS) :)
(: We can only check that when ACTARM in ('Screen Failure' 'Not Assigned' 'Not  Treated') then RFSTDTC = null :)
xquery version "3.0";
...
for $record in $dmdatasetdoc//odm:ItemGroupData
    ...
    return <error rule="CG0145" variable="RFSTDTC" dataset="DM" rulelastupdate="2020-06-14" recordnumber="{data($recnum)}">RFSTDTC='{data($rfstdtcvalue)}' expected to be null for ACTARM='{data($actarmvalue)}'</error>						
