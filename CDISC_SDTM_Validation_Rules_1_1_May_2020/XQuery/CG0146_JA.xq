...
for $record in $dmdatasetdoc//odm:ItemGroupData
    ...
    (: when ACTARM not in ('Screen Failure' 'Not Assigned' 'Not  Treated') then RFSTDTC != null :)
    where not($actarmvalue='Screen Failure' or $actarmvalue='Not Assigned' or $actarmvalue='Not  Treated') and not($rfstdtcvalue)
    return <error rule="CG0146" variable="RFSTDTC" dataset="DM" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">RFSTDTC expected to be not null for ACTARM='{data($actarmvalue)}'</error>			
