...
for $lbdatasetdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'LB') or @Domain='LB']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$lbtoxgroid and @Value]]
        ...
        return <error rule="CG0185" dataset="{data($name)}" variable="LBTOXGR" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Value of LBTOXGR={data($lbtoxgr)} represents a numeric scale but is not a complete number</error>			
