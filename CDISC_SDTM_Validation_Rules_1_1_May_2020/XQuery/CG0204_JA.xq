...
for $suppitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'SUPP')]
    ...
    for $record in $suppdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$idvaroid and @Value!='']] (: IDVAR is not null :)
        ...
        return <error rule="CG0204" dataset="{data($name)}" variable="IDVARVAL" recordnumber="{data($recnum)}" rulelastupdate="2020-06-15" >IDVARVAL is not allowed to be null when IDVAR='{data($idvar)}' is not null</error>			
