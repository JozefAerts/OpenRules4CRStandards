...
(: and the dataset location and the document (when present) :)
...
for $record in $sedatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$etcdoid and @Value='UNPLAN']]
    ...
    return <error rule="CG0206" dataset="SE" rulelastupdate="2020-08-04" variable="TAETORD" recordnumber="{data($recnum)}" >A non-null value for TAETORD={data($taetord)} was found in combination with ETCD='UNPLAN'</error>	
