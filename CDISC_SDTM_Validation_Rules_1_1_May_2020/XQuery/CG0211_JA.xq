...
for $record in $sedatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$seupdesoid]/@Value]
    ...
    return <error rule="CG0211" dataset="SE" variable="ETCD" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">When SEUPDES!=null then ETCD is expected to be 'UNPLAN'. ETCD='{data($etcd)}', SEUPDES='{data($seupdes)}' was found</error>				
