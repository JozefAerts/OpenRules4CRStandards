...
for $record in $svdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$svtaetordoid]]
    ...
    for $serecord in $sedatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$seusubjidoid and @Value=$svusubjid]][odm:ItemData[@ItemOID=$sestdtcoid] and odm:ItemData[@ItemOID=$seendtcoid]]
        ...
        return <error rule="CG0217" dataset="SV" variable="TAETORD" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">TAETORD is present in SV dataset and SESTDTC &lt;= SVSTDTC and SVSTDTC &lt;= SEENDTC, but SV.TAETORD={data($svtaetord)} is not equal to SE.TAETORD={data($setaetord)}</error>				
