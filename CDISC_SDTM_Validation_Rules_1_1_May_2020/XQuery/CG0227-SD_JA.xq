...
for $datasetdef in doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='DM')][@Name=$datasetname]
    ...
    for $record in $datasetdoc[$enrfoid and $usubjidoid]//odm:ItemGroupData
        ...
        return <error rule="CG0227" dataset="{data($name)}" variable="{data($name)}" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">{data($enrfname)}={data($enrf)} is found although RFENDTC in DM is null</error>			
