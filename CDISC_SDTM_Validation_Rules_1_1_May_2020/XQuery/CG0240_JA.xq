...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef
    ...
    for $tptnumuniquevalue in $tptnumuniquevalues
        ...
        return <error rule="CG0240" dataset="{data($name)}" variable="{data(tptname)}" rulelastupdate="2020-06-15">Inconsistent value for {data($tptname)} within {data($tptnumname)} in dataset {data($datasetname)}. {data($uniquetptvaluescount)} different {data($tptname)} values were found for {data($tptnumname)}={data($tptnumuniquevalue)}</error>						
