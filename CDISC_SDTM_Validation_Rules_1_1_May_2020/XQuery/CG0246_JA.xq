...
let $name := $datasetdef/@Name
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0246" dataset="{data($name)}" variable="ETCD" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">Invalid value for ETCD dataset {data($name)} - Value {data($etcdvalue)} has more than 8 characters</error>
