...
for $record in $tadatasetdoc//odm:ItemGroupData
    ...
    for $recordnext in $record/following-sibling::odm:ItemGroupData[odm:ItemData[@ItemOID=$armoid and @Value=$arm]]
        ...
        return <error rule="CG0247" dataset="TA" variable="TAETORD" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">Records {data($recnum)} and {data($recnum2)} have the same value for ARM='{data($arm)}' and TAETORD={data($taetord)}</error>
