...
for $record in $tidatasetdoc[$ietestcdoid and $tiversoid]//odm:ItemGroupData
    ...
    for $recordnext in $record/following-sibling::odm:ItemGroupData[odm:ItemData[@ItemOID=$tiversoid and @Value=$tivers]]
        ...
        return <error rule="CG0255" dataset="TI" variable="IETESTCD" rulelastupdate="2020-06-15" recordnumber="{data($recnum)}">Records {data($recnum)} and {data($recnumnext)} have the same value for IETESTCD='{data($ietestcd)}' and TIVERS={data($tivers)}</error>			
