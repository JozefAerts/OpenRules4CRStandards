...
(: Rule CG0257 - TSPARMCD value length <= 8 :)
xquery version "3.0";
...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0257" dataset="TS" variable="TSPARMCD" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">The value for TSPARMCD in dataset {data($datasetname)} has more than 8 characters: '{data($testvalue)}' was found</error>			
