...
(: Rule CG0258 - TSPARM value length <= 40 :)
xquery version "3.0";
...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        let $testvalue := $record/odm:ItemData[@ItemOID=$tsparmoid]/@Value
        (: when it has more than 40 characters, give an error :)
        where string-length($testvalue) > 40
        return <error rule="CG0258" dataset="TS" variable="TSPARM" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">The value for TSPARM in dataset {data($datasetname)} has more than 40 characters: '{data($testvalue)}' was found</error>			
