...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0258" dataset="TS" variable="TSPARM" recordnumber="{data($recnum)}" rulelastupdate="2020-06-15">The value for TSPARM in dataset {data($datasetname)} has more than 40 characters: '{data($testvalue)}' was found</error>			
