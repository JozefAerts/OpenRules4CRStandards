...
for $datasetdef in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $tsvalcduniquevalue in $tsvalcduniquevalues
        ...
        return <error rule="CG0265" dataset="{data($name)}" variable="TSVALCD" rulelastupdate="2020-06-15">Inconsistent value for TSVAL within TSVALCD in dataset {data($datasetname)}. {data($uniquetsvalvaluescount)} different TSVAL values were found for TSVALCD={data($tsvalcduniquevalue)}</error>	
