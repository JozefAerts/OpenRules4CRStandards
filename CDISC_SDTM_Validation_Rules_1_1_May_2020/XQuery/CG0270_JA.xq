...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsvaloid] and odm:ItemData[@ItemOID=$tsparmcdoid and @Value='AGEMAX']]
        ...
        return <error rule="CG0270" dataset="TS" variable="TSVAL" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">TSVAL={data($tsval)} for TSPARMCD='AGEMAX' is expected to be an ISO-8601 duration</error>				
