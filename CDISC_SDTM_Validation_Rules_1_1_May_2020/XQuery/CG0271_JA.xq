...
(: Rule CG0271 - When TSPARMCD = 'RANDOM' and TSVAL != null then TSVAL in ('N','Y') :)
xquery version "3.0";
...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='RANDOM']]
        ...
        return <error rule="CG0271" dataset="TS" variable="TSVAL" recordnumber="{data($recnum)}" rulelastupdate="2020-06-15">TSVAL='{data($tsval)}' for TSPARMCD='RANDOM' is expected to be in ('N','Y')</error>							
