...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='RANDOM']]
        ...
        return <error rule="CG0271" dataset="TS" variable="TSVAL" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">TSVAL='{data($tsval)}' for TSPARMCD='RANDOM' is expected to be in ('N','Y')</error>							
