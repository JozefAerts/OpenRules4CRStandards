...
(: Rule CG0275 - When TSPARMCD = 'CURTRT' and record exists where TSPARMCD = 'ADDON' and TSVAL = 'Y' then TSVAL != null :)
xquery version "3.0";
...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    (: TSPARMCD = 'CURTRT' TSVAL must not be null :)
    where $addonyesrecord and (not($curtrtvalue))  (: give an error when TSVAL for TSPARMCD=CURTRT is null  :)
    return <error rule="CG0275" dataset="TS" variable="TSVAL" rulelastupdate="2020-06-15">Null value for TSVAL for TSPARMCD='CURTRT' found although a record with TSPARMCD='ADDON and TSVAL='Y' exists</error>			
