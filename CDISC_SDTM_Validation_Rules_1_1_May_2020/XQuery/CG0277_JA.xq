...
:)
	...
	(: Rule CG0277 - When TSPARMCD = 'INTMODEL' and record exists where TSPARMCD = 'STYPE' and TSVAL = 'INTERVENTIONAL' then TSVAL != null :)
xquery version "3.0";
...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    (: look for a record with TSPARMCD='INTMODEL' :)
    let $trtrecord :=  $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='INTMODEL']]
    let $recnum := $trtrecord/@data:ItemGroupDataSeq
    let $trtvalue := $trtrecord/odm:ItemData[@ItemOID=$tsvaloid]/@Value
    (: TSVAL for TSPARMCD = 'INTMODEL' must not be null :)
    where $interventionalrecord and (not($trtvalue))  (: give an error when TSVAL for TSPARMCD=INTMODEL is null  :)
    return <error rule="CG0277" dataset="TS" variable="TSVAL" rulelastupdate="2020-06-15">Null value for TSVAL for TSPARMCD='INTMODEL' found although a record with TSPARMCD='STYPE and TSVAL='INTERVENTIONAL' exists</error>		
