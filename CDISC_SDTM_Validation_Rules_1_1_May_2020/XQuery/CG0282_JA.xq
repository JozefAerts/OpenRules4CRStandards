...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
   ...
   for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='ADAPT']]
        ...
        return <error rule="CG0282" dataset="TS" variable="TSVAL" recordnumber="{data($recnum)}" rulelastupdate="2020-06-15">TSVAL='{data($tsval)}' for TSPARMCD='ADAPT' is expected to be in ('N','Y')</error>				
