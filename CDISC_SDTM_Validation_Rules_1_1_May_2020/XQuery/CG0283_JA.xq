...
for $itemgroup in doc(concat($base,$define))//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='DCUTDTC']]
        ...
        return <error rule="CG0283" dataset="TS" variable="TSVAL" record="{data($recnum)}" rulelastupdate="2020-06-15">TSVAL={data($tsval)} for TSPARMCD=DCUTDTC is expected to be a valid ISO-8601 date</error>				
