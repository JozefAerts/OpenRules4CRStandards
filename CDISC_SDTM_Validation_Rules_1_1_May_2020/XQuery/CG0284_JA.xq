...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='NARMS']]
        ...
        return <error rule="CG0284" dataset="TS" variable="TSVAL" record="{data($recnum)}" rulelastupdate="2020-08-04">TSVAL={data($tsval)} for TSPARMCD=NARMS is expected to be an integer &gt; 0</error>				
