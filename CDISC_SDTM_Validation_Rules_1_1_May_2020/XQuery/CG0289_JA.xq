...
(: Method 2: Uses XML4Pharma RESTful web services (CDISC Library in future)  - enforce XML :)
let $webservice := 'http://www.xml4pharmaserver.com:8080/CDISCCTService2/rest/CodeListVersions/SDTM.xml'
...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='TS']
    ...
    for $tsvcdver in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsvcdrefoid and @Value='CDISC']]/odm:ItemData[@ItemOID=$tsvcdveroid]/@Value
	    ...
	    return  <error rule="CG0289" dataset="TS" variable="TSVCDVER" rulelastupdate="2020-08-04">The Value TSVCDVER='{data($tsvcdver)}' for TSVCDREF='CDISC' is not recognized as a valid CDISC-CT version from the list ({data($cdiscctversions)})</error>
