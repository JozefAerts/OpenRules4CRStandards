...
for $taitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'FA')]
    ...
    return <error rule="CDG0333" dataset="{data($name)}" rulelastupdate="2020-08-04">No corresponding dataset or domain '{data($aboutdomain)}' was found for Findings About dataset '{data($name)}'</error>			
