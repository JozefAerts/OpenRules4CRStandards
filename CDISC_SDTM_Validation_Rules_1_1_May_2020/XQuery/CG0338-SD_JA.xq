...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$scatoid] and odm:ItemData[@ItemOID=$decodoid]]
        ...
        where $scat=$decod
        return <error rule="CG0338" dataset="{data($name)}" variable="{data($scatname)}" recordnumber="{data($recnum)}" rulelastupdate="2020-06-17">Value of {data($scatname)}='{data($scat)}' is not allowed to be equal to the value of {data($decodname)}='{data($decod)}'</error>			
