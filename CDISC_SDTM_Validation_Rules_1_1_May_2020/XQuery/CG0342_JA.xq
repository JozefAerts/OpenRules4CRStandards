...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData/@ItemOID=$trtoid]
        ...
        return <error rule="CG0342" dataset="{data($name)}" variable="{data($trtname)}" recordnumber="{data($recnum)}" rulelastupdate="2020-06-17">Value of {data($trtname)}='{data($trt)}' is not allowed to be equal to 'OTHER'</error>						
