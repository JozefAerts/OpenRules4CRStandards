...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    where $dthrelname
    return <error rule="CG0352" dataset="{data($name)}" variable="{data($dthrelname)}" rulelastupdate="2020-08-04">The use of the variable '{data($dthrelname)}' is not allowed in SDTM (it is specific to SEND)</error>			
