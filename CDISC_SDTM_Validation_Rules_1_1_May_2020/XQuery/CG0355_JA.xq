...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    where $detectname
    return <error rule="CG0355" dataset="{data($name)}" variable="{data($detectname)}" rulelastupdate="2020-08-04">The use of the variable '{data($detectname)}' is not allowed in SDTM (it is specific to SEND)</error>			
