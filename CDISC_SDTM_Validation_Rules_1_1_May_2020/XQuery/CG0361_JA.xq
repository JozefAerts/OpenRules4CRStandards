...
let $relsubitemgroupdef := $definedoc//odm:ItemGroupDef[@Name='RELSUB']
    ...
    for $record in $relsubdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$usubjidoid]]
        ...
        (: POOLID must be null :)
        where $poolid
        return <error rule="CG0361" dataset="RELSUB" variable="POOLID" recordnumber="{data($recnum)}" rulelastupdate="2020-06-17">POOLID='{data($poolid)}' must be null as USUBJID='{data($usubjid)}' is not null</error>		
