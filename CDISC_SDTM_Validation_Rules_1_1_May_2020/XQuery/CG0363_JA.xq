...
(: Rule CG0363 - When RSUBJID != null then RSUBJID != USUBJID :)
xquery version "3.0";
...
let $relsubitemgroupdef := $definedoc//odm:ItemGroupDef[@Name='RELSUB']
    (: and get the OID of the USUBJID and of RSUBJID variable (if any) :)
    let $usubjidoid := (
	...
	let $relsubdatasetlocation := (
		if($defineversion='2.1') then $relsubitemgroupdef/def21:leaf/@xlink:href
		else $relsubitemgroupdef/def:leaf/@xlink:href
	)
    let $relsubdoc := (
    ...
    for $record in $relsubdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$rsubjidoid]]
        ...
        (: get the value of RSUBJID and of USUBJID (if any) :)
        let $usubjid := $record/odm:ItemData[@ItemOID=$usubjidoid]/@Value
        let $rsubjid := $record/odm:ItemData[@ItemOID=$rsubjidoid]/@Value
        (: RSUBJID != USUBJID :)
        where $usubjid=$rsubjid
        return <error rule="CG0363" dataset="RELSUB" variable="RSUBJID" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">USUBJID='{data($usubjid)}' must be different from RSUBJID='{data($rsubjid)}'</error>						
