...
let $relsubitemgroupdef := $definedoc//odm:ItemGroupDef[@Name='RELSUB']
    ...
    for $record in $relsubdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$rsubjidoid]]
        ...
        return <error rule="CG0364" dataset="RELSUB" variable="RSUBJID" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">POOLID='{data($poolid)}' must be different from RSUBJID='{data($rsubjid)}'</error>						
