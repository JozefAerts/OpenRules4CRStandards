...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name='CO' or @Name='RELREC' or starts-with(@Name,'SUPP')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$rdomainoid] and odm:ItemData[@ItemOID=$idvaroid]]
        ...
        return <error rule="CG0370" dataset="{data($name)}" variable="RDOMAIN" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">Variable '{data($idvarvalue)}' was not found in the parent dataset of the referenced domain RDOMAIN='{data($rdomain)}' </error>
