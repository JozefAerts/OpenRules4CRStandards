...
let $datasets := doc(concat($base,$define))//odm:ItemGroupDef[@Name='RELREC' or starts-with(@Name,'CO') or starts-with(@Name,'SUPP')][@Name=$domain]
...
for $dataset in $datasets
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0371" dataset="{data($name)}" variable="IDVARVAL" rulelastupdate="2020-06-17" recordnumber="{data($recnum)}">Invalid referenced record - Dataset={data($name)}: no parent records found in RDOMAIN={data($rdomainval)} for USUBJID={data($usubjidvalue)} with IDVAR='{data($idvarvalue)}' and IDVARVAL='{data($idvarvalvalue)}'</error>			
