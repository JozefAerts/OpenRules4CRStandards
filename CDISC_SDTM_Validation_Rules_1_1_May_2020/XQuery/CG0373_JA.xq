...
let $suppdatasets := doc(concat($base,$define))//odm:ItemGroupDef[starts-with(@Name,'SUPP')]
...
for $itemgroupdef in $suppdatasets
    ...
    for $rdomain in $domainvalues
        ...
        where $count = 0
        return <error rule="CG0373" dataset="{data($name)}" rulelastupdate="2020-06-18">No parent dataset/domain was found for RDOMAIN='{data($rdomain)}'</error>			
