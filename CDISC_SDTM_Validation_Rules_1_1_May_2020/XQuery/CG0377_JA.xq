...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        return <error rule="CG0377" dataset="{data($name)}" variable="{data($lltname)}" recordnumber="{data($recnum)}" rulelastupdate="2020-06-18">Value '{data($lltvalue)}' for {data($lltname)} was not found in the MedDRA dictionary</error>						
