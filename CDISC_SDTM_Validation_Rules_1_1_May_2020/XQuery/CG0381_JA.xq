...
(: Rule CG0381: --HLT = MedDRA high level term :)
xquery version "3.0";
...
(: get the high level term, which is the second field - field separator is the $ character :)
let $hltterms := (
	for $line in $lines
	return upper-case(tokenize($line,'\$')[2])
)  
(: $hltterms know contains all the MedDRA HLT terms in uppercase , e.g.: 'Diarrhoea (excl infective' :)
...
for $itemgroup in $definedoc//odm:ItemGroupDef[@Name='AE' or @Name='MH' or @Name='CE' or @Domain='AE' or @Domain='MH' or @Domain='CE']
    ...
    let $hltoid := (
        for $a in $definedoc//odm:ItemDef[ends-with(@Name,'HLT')]/@OID 
    ...
    for $record in $datasetdoc//odm:ItemGroupData
        ...
        (: give an error when the HLT value is not one of the HLT terms of MedDRA -  :)
        where $hltvalue and not(functx:is-value-in-sequence(upper-case($hltvalue),$hltterms))
        return <error rule="CG0381" dataset="{data($name)}" variable="{data($hltname)}" recordnumber="{data($recnum)}" rulelastupdate="2019-08-12">Value '{data($hltvalue)}' for {data($hltname)} was not found in the MedDRA dictionary as a high level term</error>			
