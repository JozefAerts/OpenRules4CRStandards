...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aeseroid]]
        ...
        return <error rule="CG0387" dataset="{data($name)}" variable="AESER" recordnumber="{data($recnum)}" rulelastupdate="2020-06-18">Value of AESER='{data($aeser)}' must be one of 'Y' or 'N'</error>						
