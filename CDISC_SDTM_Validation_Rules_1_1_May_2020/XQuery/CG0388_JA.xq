...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aescanoid]]
        ...
        return <error rule="CG0388" dataset="{data($name)}" variable="AESCAN" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">Value of AESCAN='{data($aescan)}' must be one of 'Y' or 'N'</error>						
