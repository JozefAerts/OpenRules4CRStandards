...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aescongoid]]
        ...
        return <error rule="CG0389" dataset="{data($name)}" variable="AESCONG" recordnumber="{data($recnum)}" rulelastupdate="2020-06-18">Value of AESCONG='{data($aescong)}' must be one of 'Y' or 'N'</error>						
