...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aesdisaboid]]
        ...
        return <error rule="CG0390" dataset="{data($name)}" variable="AESDISAB" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">Value of AESDISAB='{data($aesdisab)}' must be one of 'Y' or 'N'</error>						
