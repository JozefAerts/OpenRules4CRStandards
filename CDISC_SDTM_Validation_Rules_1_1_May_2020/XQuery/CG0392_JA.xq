...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aeshsopoid]]
        ...
        return <error rule="CG0392" dataset="{data($name)}" variable="AESHOSP" recordnumber="{data($recnum)}" rulelastupdate="2020-06-18">Value of AESHOSP='{data($aeshsop)}' must be one of 'Y' or 'N'</error>						
