...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aesodoid]]
        ...
        return <error rule="CG0394" dataset="{data($name)}" variable="AESOD" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">Value of AESOD='{data($aesod)}' must be one of 'Y' or 'N'</error>						
