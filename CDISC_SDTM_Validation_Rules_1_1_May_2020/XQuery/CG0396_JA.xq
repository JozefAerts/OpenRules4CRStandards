...
for $aeitemgroupdef in $definedoc//odm:ItemGroupDef[starts-with(@Name,'AE')]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$aecontrtoid]]
        ...
        return <error rule="CG0396" dataset="{data($name)}" variable="AECONTRT" recordnumber="{data($recnum)}" rulelastupdate="2020-06-18">Value of AECONTRT='{data($aecontrt)}' must be one of 'Y' or 'N'</error>						
