...
for $itemgroupdef in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname]
    ...
    for $d in doc($datasetpath)//odm:ItemGroupData/odm:ItemData[@ItemOID=$studyoid]
        ...
        return <error rule="CG0409" dataset="{data($name)}" variable="STUDYID" rulelastupdate="2020-06-18" recordnumber="{data($recordnumber)}">STUDYID {data($d/@Value)} in dataset {data($dataset)} does not match the STUDYID {data($dmstudyid)} in the DM dataset</error>	
