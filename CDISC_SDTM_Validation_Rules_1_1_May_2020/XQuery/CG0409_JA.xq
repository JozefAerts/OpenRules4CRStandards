...
for $itemgroupdef in $definedoc//odm:ItemGroupDef
    ...
    for $d in $datasetdoc//odm:ItemGroupData/odm:ItemData[@ItemOID=$studyoid]
        ...
        return <error rule="CG0409" dataset="{data($datasetname)}" variable="STUDYID" rulelastupdate="2020-08-04" recordnumber="{data($recordnumber)}">STUDYID {data($d/@Value)} in dataset {data($dataset)} does not match the STUDYID {data($dmstudyid)} in the DM dataset</error>	
