...
let $datasets := doc(concat($base,$define))//odm:ItemGroupDef[not(@Name='TA')][@Name=$datasetname]
for $dataset in $datasets
    ...
    return <error rule="CG0418" dataset="TA" variable="TAETORD" rulelastupdate="2020-06-18" recordnumer="{data($recnum)}">Invalid TAETORD in {data($name)} in dataset {data($datasetname)}. TAETORD={data($taetord)} was not found in dataset {data($tadatasetname)}</error>	
