...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name=$datasetname][upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS']
    ...
    for $record in $datasetdoc[$occuroid and $enrfoid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$occuroid and @Value='N']]
        ...
        return <error rule="CG0421" dataset="{data($name)}" variable="{data($enrfname)}" rulelastupdate="2020-06-19" recordnumer="{data($recnum)}">{data($occurname)}='N' but {data($enrfname)} is not null. {data($enrfname)}='{data($enrf)}' is found</error>							
