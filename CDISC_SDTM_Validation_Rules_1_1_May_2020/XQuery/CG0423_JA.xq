...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' or upper-case(./def21:Class/@Name)='INTERVENTIONS']
    ...
    for $record in $datasetdoc[$vamtoid and $trtvoid]//odm:ItemGroupData[odm:ItemData[@ItemOID=$vamtoid]]
        ...
        return <error rule="CG0423" dataset="{data($name)}" variable="{data($trtvname)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">{data($trtvname)} may not be NULL, as ({data($vamtname)})='{data($vamt)}' is not null</error>			
