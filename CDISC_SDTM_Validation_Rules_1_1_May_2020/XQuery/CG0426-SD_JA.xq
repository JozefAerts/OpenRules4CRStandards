...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS' or upper-case(./def21:Class/@Name)='FINDINGS'][@Domain=$datasetname or @Name=$datasetname]
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$stresuoid]]
        ...
        where not($stresc)
        return <error rule="CG0426" dataset="{data($name)}" variable="{data($strescname)}" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">{data($stresuname)}='{data($stresu)}' is not NULL, but {data($strescname)}=NULL</error>			
