...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$stresuoid]]
        ...
        where not($stresc)
        return <error rule="CG0426" dataset="{data($name)}" variable="{data($strescname)}" rulelastupdate="20-06-19" recordnumber="{data($recnum)}">{data($stresuname)}='{data($stresu)}' is not NULL, but {data($strescname)}=NULL</error>			
