...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$ageuoid]]
        ...
        (: When AGEU != null and AGE = null then AGETXT != null :)
        where not($agevalue) and not($agetxtvalue)
        return <error rule="CG0434" dataset="DM" variable="AGE" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">AGETXT=null where AGE=null and AGEU='{data($ageuvalue)}' is not null</error>				
