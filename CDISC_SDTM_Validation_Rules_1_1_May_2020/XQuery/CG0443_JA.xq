...
for $record in $tsdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and @Value='CURTRT']]
    ...
    where $uniicount=0
    return <error rule="CG0443" dataset="TS" variable="TSVALCD" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid TSVALCD value={data($tsvalcdvalue)} for TSPARMCD='CURTRT'</error>			
