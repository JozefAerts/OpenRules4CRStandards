...
(: Rule CG0445 - When TSPARMCD = 'COMPTRT' then TSVAL is a valid preferred term from FDA Substance Registration System (SRS) :)
xquery version "3.0";
...
for $record in doc($tsdatasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid]][1]
    ...
    (: TODO: this does not work - one ALSO needs to provide the UNII - active_moiety is optional :)
    ...
    :)
    where $uniicount=0
    return <error rule="CG0445" dataset="TS" variable="TSVAL" rulelastupdate="2020-06-19" recordnumber="{data($recnum)}">Invalid TSVAL value={data($tsvalvalue)} for TSPARMCD=COMPTRT in dataset TS</error>
