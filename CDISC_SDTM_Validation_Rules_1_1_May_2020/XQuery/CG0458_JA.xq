...
for $record in $tsdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$tsparmcdoid and(@Value='INDIC' or @Value='TDIGRP')]]
    ...
    return <error rule="CG0458" dataset="TS" variable="TSVCDREF" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">Invalid TSVCDREF value '{data($tsvcdrefvalue)}' for Trial Summary Parameter TSPARMCD={data($tsparmcdvalue)} in dataset {data($tsdatasetname)}</error>				
