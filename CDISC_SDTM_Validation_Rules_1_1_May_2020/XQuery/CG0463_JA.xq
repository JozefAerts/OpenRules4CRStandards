...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='INTERVENTIONS' and not(functx:is-value-in-sequence(substring(@Name,1,2),$interventiondomains))]
    ...
    (: --TRT must be defined :)
    where not($trtoid)
    return <error rule="CG0463" dataset="{data($name)}" variable="{data($varname)}" rulelastupdate="2020-06-19">{data($varname)} is not present in FINDINGS custom domain {data($name)}</error>										
