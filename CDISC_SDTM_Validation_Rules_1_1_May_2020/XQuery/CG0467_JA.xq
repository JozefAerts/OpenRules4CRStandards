...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS'or upper-case(./def21:Class/@Name)='FINDINGS']
    ...
    where $stdtcoid
    return <error rule="CG0467" dataset="{data($name)}" variable="" rulelastupdate="2020-08-04">Variable {data($stdtcname)} is not allowed to be present in the FINDINGS dataset {data($name)}</error>													
