...
(: Rule CG0513: DM: When ACTARMCD = null, then ARMNRS^=null :)
xquery version "3.0";
...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    (: get the OID of the ACTARMCD and of ARMNRS :)
    let $actarmcdoid := (
    ...
    for $record in $datasetdoc//odm:ItemGroupData[not(odm:ItemData[@ItemOID=$actarmcdoid])]
        ...
        return <error rule="CG0513" dataset="DM" variable="ACTARMCD" rulelastupdate="2020-08-04" recordnumber="{data($recnum)}">ARMNRS is not populated although ACTARMCD is null</error>
