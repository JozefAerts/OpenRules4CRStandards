...
for $dmrecord in $dmdatasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID='$raceoid' and @Value='MULTIPLE']]
    ...
    return <error rule="CG0531" variable="RACE" dataset="DM" rulelastupdate="2020-06-21" recordnumber="{data($recnum)}">Only {data($suppdmcountrace)} 'RACE' records for USUBJID='{data($usubjid)}' have been found in SUPPDM for DM.RACE='{data($race)}'</error>				
