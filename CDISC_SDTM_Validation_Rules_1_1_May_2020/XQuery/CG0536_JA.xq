...
for $group in $orderedrecords
	...
	return <error rule="CG0536" dataset="DS" rulelastupdate="2020-08-04">There is more than 1 record ({count($group/odm:ItemGroupData)} records found) for the combination of USUBJID='{data($group/odm:ItemGroupData[1]/odm:ItemData[@ItemOID=$usubjidoid]/@Value)}', DSSCAT='{data($group/odm:ItemGroupData[1]/odm:ItemData[@ItemOID=$dsscatoid]/@Value)}' and EPOCH='{data($group/odm:ItemGroupData[1]/odm:ItemData[@ItemOID=$epochoid]/@Value)}'</error>
