...
for $group in $orderedrecords
    ...
    for $record in $group/odm:ItemGroupData
        ...
        return <error rule="CG0538" dataset="DS" recordnumber="{data($recnum)}" rulelastupdate="2020-08-04">There is more than 1 record ({$countstudyparticipationrecords} records were found) with DSSCAT='STUDY PARTICIPATION' although there are more than 1 records with the same value for EPOCH='{data($epochvalue)}' and DSCAT='DISPOSITION EVENT' for subject with USUBJID='{data($usubjidvalue)}'</error>	
