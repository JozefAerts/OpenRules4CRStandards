...
(: Rule CG0539: When Subject has more than one record per Epoch with DSCAT = 'DISPOSITION EVENT', 
...
for $group in $orderedrecords
    ...
    there may be only 1 record for the USUBJID with DSSCAT='STUDY TREATMENT' :)
    ...
    for $record in $group/odm:ItemGroupData
        ...
        that have DSSCAT='STUDY TREATMENT':)
        let $countstudytreatmentrecords := count($dsdoc/odm:ItemGroupData[odm:ItemData[@ItemOID=$usubjidoid and @Value=$usubjidvalue]]/odm:ItemData[@ItemOID=$dsscatoid and @Value='STUDY TREATMENT'])
