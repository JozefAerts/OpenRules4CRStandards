...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS' or upper-case(./def21:Class/@Name)='FINDINGS']
    ...
    for $record in $datasetdoc[$lobxfloid]//odm:ItemGroupData
        ...
        return <error rule="CG0541" dataset="{data($name)}" recordnumber="{data($recnum)}" variable="{data($lobxflname)}" rulelastupdate="2020-08-04">Value of {data($lobxflname)} is not null or 'Y' - value {data($lobxflvalue)}</error>	
