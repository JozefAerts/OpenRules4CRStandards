...
(: Rule CG0544: SE, PR, SM: --SEQ is in consistent chronological order :)
...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[@Name='SE' or @Name='PR' or @Name='SM']
    ...
    for $group in $groupedrecords
        ...
        where not(functx:sequence-deep-equal($stdtcdates,$sortedrecordsingroup))
       	return <error rule="CG0544" dataset="{data($name)}" rulelastupdate="2020-06-22">Records for subject with USUBJID={data($usubjidvalue)} are not in correct chronological order by {data($seqname)} and {data($stdtcname)}</error>		
