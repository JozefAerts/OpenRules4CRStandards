...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='FINDINGS']
    ...
    for $record in $datasetdoc//odm:ItemGroupData[odm:ItemData[@ItemOID=$strefcoid]]
        ...
        return <error rule="CG0550" dataset="{data($name)}" variable="{data($strefnname)}" recordnumber="{data($recnum)}" rulelastupdate="2020-06-22">{data($strefnname)} is not populated although {data($strefcname)} is populated</error>							
