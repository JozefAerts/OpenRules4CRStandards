...
for $itemgroupdef in $definedoc//odm:ItemGroupDef[upper-case(@def:Class)='EVENTS' or upper-case(./def21:Class/@Name)='EVENTS' and not(@Name='MH') and not(@domain='MH')]
    ...
    where $evdtypoid
    return <error rule="CG0567" dataset="{data($name)}" rulelastupdate="2020-08-04">Variable {data($evdtypname)} is not allowed to be used in SDTM submissions</error>				
