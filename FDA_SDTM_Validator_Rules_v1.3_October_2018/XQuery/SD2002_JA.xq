...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmcdoid]]
        ...
        return <error rule="SD2002" dataset="DM" variable="ACTARMCD" rulelastupdate="2020-08-08" recordnumber="{data($recnum)}">Invalid value for ACTARMCD, value={data($actarmcdvalue)} in dataset DM is not one of ('SCRNFAIL', 'NOTASSGN', 'NOTTRT', 'UNPLAN') and is not found in TA.ARMCD</error>			
