...
for $dataset in $definedoc//odm:ItemGroupDef[@Name='DM']
    ...
    for $record in doc($datasetlocation)//odm:ItemGroupData[odm:ItemData[@ItemOID=$actarmoid]]
        ...
        return <error rule="SD2003" dataset="DM" variable="ACTARM" rulelastupdate="2020-08-08" recordnumber="{data($recnum)}">Invalid value for ACTARM, value={data($actarmvalue)} in dataset {data($datasetlocation)}</error>			
