...
for $dataset in doc(concat($base,$define))//odm:ItemGroupDef[@Name=$datasetname][upper-case(@def:Class)='INTERVENTIONS' or upper-case(@def:Class)='EVENTS' or upper-case(@def:Class)='FINDINGS' or @Domain='SE' or @Domain='SV']
    let $name := $dataset/@Name
    let $ds := $dataset/def:leaf/@xlink:href
    let $datasetlocation := concat($base,$ds)
