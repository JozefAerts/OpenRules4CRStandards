...
let $evlintoid := (
    for $a in doc(concat($base,$define))//odm:ItemDef[ends-with(@Name,'EVLINT')]/@OID 
    where $a = $dataset/odm:ItemRef/@ItemOID
    return $a
)
let $evlintname := doc(concat($base,$define))//odm:ItemDef[@OID=$evlintoid]/@Name
...
for $record in doc($datasetlocation)[$duroid or $eltmoid or $evlintoid]//odm:ItemGroupData
    ...
    for $datapoint in $durdatapoints
        ...
        let $itemname := doc(concat($base,$define))//odm:ItemDef[@OID=$itemoid]/@Name
